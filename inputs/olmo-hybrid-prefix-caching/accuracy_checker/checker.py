"""
Accuracy checker: verify that the custom Olmo-Hybrid model implementation
...
Usage:
    CUDA_VISIBLE_DEVICES=0 .venv/bin/python checker.py
    CUDA_VISIBLE_DEVICES=0 .venv/bin/python checker.py --model-dir ../model
"""
...
# ---------------------------------------------------------------------------
# Test samples — base-model style completions. Olmo-Hybrid-7B is a base
# (non-chat) model, so we don't rely on a chat template.
# ---------------------------------------------------------------------------
