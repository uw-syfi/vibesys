...
- **A checkpoint-interval knob doesn't raise the hit rate if something else caps the reusable prefix upstream.** The "every N tokens" stride above only gates what a single insert can commit; shrinking N does nothing measurable if the true limiter sits earlier in the pipeline. Measured case: one implementation's mamba-cache checkpoint interval (SGLang's `--mamba-track-interval`) at 256, 128, and 64 gave identical cached-token fractions (0.571 / 0.572 / 0.577) and no TTFT change, because the actual cap on reusable prefix was a chat-template mismatch upstream of the cache entirely (see `radix-prefix-caching.md`'s chat-template pitfall), not the checkpoint stride. Diagnose what the client actually re-sends before tuning a checkpoint-interval knob for hit rate. Scope: sglang, hybrid Mamba/GDN radix cache. Status: refuted (measured). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-12, job 633710.
