...
### TTFT track: campaign status

**Closing numbers.** Against the pre-campaign baseline (`baseline_v1cfg`, all accepted changes off, see "Load dependence" above): pooled p95 TTFT turn2+ at 48 uncapped sessions has moved from 922 ms to about 207 ms, and mean TPOT from 175 ms to about 10.1 ms, on the accepted stack described in the "Current accepted numbers" paragraph above.

**Remaining gap, and why the two levers investigated for it did not ship.** GDN prefill capture (folding the 45 Gated-DeltaNet layers' chunked-scan kernels into the breakable prefill CUDA graph, instead of breaking the graph once per layer) is rejected as designed, not merely unfinished: an integration attempt reduced graph breaks from 60 to 15 and measured a real but smaller-than-predicted forward-wall saving (2.1-6.9 ms, not the predicted 8-13 ms, because the static kernels always pay for a worst-case padded shape), then corrupted model output under realistic concurrency (GSM8K accuracy 62.4 percent versus an accepted 94.0 percent). See [`platforms/`](../platforms/) for the full account, the two capture-incompatibility bugs fixed along the way, and the two pitfalls it generalizes. A wider-tile MXFP4 MoE prefill kernel is refuted on paper: no register or LDS headroom exists for any wider tile at this shape, and no aiter or CK MXFP4 grouped-GEMM kernel exists for gfx942 either (see [`platforms/`](../platforms/)). Neither lever is expected to be revisited without a design change: GDN capture would need a different per-forward staleness mechanism and a concurrency-derived padding bound before it is safe to retry, and the MoE prefill kernel's design space is closed analytically, not merely untried.

**TTFT floor.** The refined current-design floor is about 180 ms p95 TTFT turn2+ at 48 sessions (measured about 207 ms, about 1.15x), against a prefill-device-time roofline of about 20 ms (17.6-22.6 ms depending on new-token count). The gap between them is queueing plus the two closed levers described above.

Status: verified (baseline and accepted numbers each independently job-verified at their own point in the campaign; the remaining-gap pointers reflect closed or rejected investigations, not open estimates). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-14, job-verified.
