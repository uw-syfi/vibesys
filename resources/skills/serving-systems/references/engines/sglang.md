...
**The earlier 3-6 ms IPC estimate was a bucket mislabel, not a measured IPC cost.** `request_built` (post-tokenize) to `sent_to_scheduler` is one coarse span in the client-facing decomposition; splitting it into 13 real sub-steps found the true pickle-plus-ZMQ-send cost near zero (the IPC row above) while the span's actual 3.44-5.76 ms (256/512/1024 extend tokens) sits inside a second, unrelated tokenization pass. `serving_chat.py` gates on `model_config.is_multimodal`, a model-capability flag true for any vision-capable checkpoint (this one included) regardless of a given request's own content, so a plain-text turn is decoded back from the `prompt_ids` `_tokenize_one_request` already computed via `tokenizer.encode`, sent to `TokenizerManager` as text, and re-encoded from scratch by the `Tokenize` stage above -- a full second tokenization pass over the same prompt on every no-media request. Candidate fix, `SGLANG_TEXT_ONLY_SEND_IDS` (send `input_ids` directly when a request carries no image/audio/video content): 0 of 253 turns mismatched against the text path on this checkpoint's tokenizer, drops the `request_built`-to-`sent_to_scheduler` span 94-96 percent (under 0.5 ms at every size). This lever interacts with a second one, suffix-only incremental tokenization (`SGLANG_INCREMENTAL_TOKENIZE`, see [`../tooling/serving-benchmark.md`](../tooling/serving-benchmark.md)): the suffix lever changes what `prompt_ids` is before the send-ids lever decides whether to forward it as-is, so the combined path needed its own exactness check before any server job, separate from each lever's own single-lever check. That combined check passed (0 of 253 turns mismatched end to end). Three paired server jobs (two c48 boot orders, one c16) then found a real but order-dependent effect: pooled p50 TTFT turn2+ improved a consistent 3.5 to 4.5 percent (4.0 to 5.7 ms) across all three jobs regardless of boot order or concurrency, but the p95 tail delta was order-dependent (-0.8 percent one c48 order, -17.7 percent the reversed order) and did not clear this campaign's own rep-spread bar in both orders; no metric regressed anywhere and median TPOT was flat. Verdict: accepted as a judgment call: the combined path is bit-exact (0 of 253), nothing regressed on any axis, and pooled median TTFT turn2+ improved by a consistent 4 to 6 ms in every job, while the p95 tail effect stayed below this campaign's own detectable delta. Both switches are now on by default for the MI300A platform through the task's platform config (env defaults, so a caller can still export 0 to turn either off), while the engine-level defaults stay off. Status: accepted, job-verified. Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13. See [`../tooling/serving-benchmark.md`](../tooling/serving-benchmark.md) for the suffix lever's own counterbalanced retest and a `--tokenizer-worker-num` pitfall found while diagnosing this span.
