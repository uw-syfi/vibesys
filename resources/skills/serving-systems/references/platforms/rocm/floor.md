...
### Optional: dword-wide packed-weight loads in the fused MXFP4 MoE kernel

Accepted on top of the permute-decode stack above: a further source-level change to the same kernel's `decode16` step reads its 16-byte packed weight block as whole dwords instead of the compiler-split eight 2-byte loads the permute fix still left in place. No new flag. Paired end to end against the permute-decode stack: median TPOT improved 30.0 percent at 48 uncapped sessions (collapse-affected reps excluded, see the pitfall note below) and 17.0 percent at a 16-session cap; pooled p95 TTFT turn2+ improved 21.8 percent and regressed a within-budget 3.4 percent respectively; bit-exact, accept_len unchanged, gates 13/13 on every rep. A minority of the 48-session job's reps on both sides hit an admission-queueing collapse independent of this kernel change; see [`../../engines/sglang.md`](../../engines/sglang.md) for that pitfall (candidate status) before reading a 48-session paired comparison's raw numbers at face value. See [`aiter-mxfp4-moe-kernel-iterations.md`](aiter-mxfp4-moe-kernel-iterations.md#h-a-dword-wide-packed-weight-loads) for the design, ISA detail, microbenchmark, and end-to-end numbers.

### Optional: fused MXFP4 MoE stage1 dispatch threshold retune, accepted at 48-session concurrency

On top of the dword-wide-loads stack above: `STAGE1_SCAFFOLD_BLOCK_THRESHOLD` moved from 1024 to 160 sorted blocks, since the dword-wide-loads change removed the scaffold-versus-template crossover the old threshold was tuned against (the templated kernel now wins at every measured block count). No new flag; a one-constant kernel-source change. Paired end to end against the dword-wide-loads stack, turn-1-admission-delay reps excluded: median TPOT improved 8.9 percent at 48 uncapped sessions (374.9 to 374.4 ms pooled p95 TTFT turn2+, flat) and 3.0 percent at a 16-session cap (295.7 to 278.9 ms pooled p95 TTFT turn2+, -5.7 percent); bf16-rounding-level dispatch-flip noise only, accept_len unchanged, gates 13/13 on every rep. Accepted as the default at 48-session concurrency, where the TPOT improvement exceeds both sides' rep-to-rep spread; not yet an unconditional default, since the 16-session TPOT improvement is smaller than rep-to-rep spread there (no regression on any metric at either concurrency). See [`aiter-mxfp4-moe-kernel-iterations.md`](aiter-mxfp4-moe-kernel-iterations.md#h-c-scaffold-versus-template-dispatch-threshold-retune-accepted-at-48-session-concurrency) for the sweep, the numerics note, and the paired acceptance numbers, and [`../../tooling/serving-benchmark.md`](../../tooling/serving-benchmark.md) for the general dispatch-threshold-staleness lesson and what the second concurrency check guards against.
...
- `PYTORCH_TUNABLEOP_VERBOSE=1`: without this, the log carries no line confirming whether a rank's table loaded, rejected, or was absent; a load-verify gate needs this var set or it can never observe a pass. See [`../../frameworks/pytorch.md`](../../frameworks/pytorch.md). Confirmed to add log output only; measured serving numbers are unchanged with it set.
...
- A TunableOp load-verify gate never passes even though the table loaded correctly; the log has no success or failure line either way. [`../../frameworks/pytorch.md#the-load-result-log-line-needs-pytorch_tunableop_verbose1-silence-is-not-failure`](../../frameworks/pytorch.md#the-load-result-log-line-needs-pytorch_tunableop_verbose1-silence-is-not-failure)
- A TunableOp table loads with no error, but no dispatched kernel ever matches a tuned solution name and GPU time never moves: CRLF line endings on the CSV's header lines silently discard the whole table. [`../../frameworks/pytorch.md#a-tunableop-results-csv-with-crlf-line-endings-silently-discards-the-whole-table`](../../frameworks/pytorch.md#a-tunableop-results-csv-with-crlf-line-endings-silently-discards-the-whole-table)
