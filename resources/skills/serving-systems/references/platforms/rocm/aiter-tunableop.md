...
### The bucketing scheme was built and its own per-bucket saving reproduced, but is refuted end to end, twice

A later campaign phase built the bucketing scheme this section only estimated: prefill M is padded up to the next multiple of 64 (`SGLANG_AITER_GEMM_PAD_M=64`, 13 buckets from 256 to 1024) and the resulting buckets tuned directly, plus an eighth shape (`mtp_fc`, the NEXTN/MTP draft model's own fc-mixing Linear) found by reading the model source. Tuning reproduced this section's own estimate cleanly at the real bucket boundaries: 46.6-74.9 percent per-bucket dense-GEMM saving across all 13 buckets (e.g. M=384: 68.5 percent, M=960: 48.4 percent), 104/104 cells hit, bf16-rounding-level exactness. `mtp_fc` tunes well too but is confirmed inert in production: it is a plain `torch.nn.Linear`, never routed through the quantized-linear path the M-padding wrapper patches.

That per-bucket saving does not survive to end-to-end single-request TTFT, refuted twice:

1. **Without a prefill CUDA graph** (held server, paired 252-row vs. 356-row TunableOp table, same commit, no `--cuda-graph-config`): at the classes this fix should improve (new/repeated prefill length, no burst/flush confound), the tuned side measured 8-9 ms *worse*, the opposite sign from the predicted 12-19 ms improvement and inside both sides' own within-class spread. Consistent with this section's own prediction: dense GEMM is too small a share (13-18 percent) of a CPU-dispatch-bound prefill window to move the wall on its own; see [`../../tooling/performance-modeling.md`](../../tooling/performance-modeling.md) for the general rule.
2. **With the accepted `breakable` prefill CUDA graph active on both sides** (removing the dispatch bottleneck that explains refutation 1): still refuted. The measured delta at the tuning's own predicted window (256-512 tokens) was only -0.6 to -3.9 ms against a predicted 12-18 ms, and deltas at M values entirely outside the 13 tuned buckets (1536/2048, where the two TunableOp CSVs are byte-identical) were *larger* (-20.3 to -34.5 ms) than at the tuned buckets themselves. No tuning-content mechanism explains an improvement at a shape neither side's table differs on; this is a boot-order confound (see [`../../tooling/serving-benchmark.md`](../../tooling/serving-benchmark.md)), not a tuning effect. A multi-rep, order-randomized design would be needed to separate a real effect from this cluster's own sequential-boot noise, which a one-run-per-side design cannot do.
3. **Retested with per-forward GPU time instead of client TTFT** (a direct read that does not need to survive host-dispatch or queueing noise): three sequential boots of the same node, accepted CSV then merged CSV then accepted CSV again (an A1/B/A2 design, the third boot serving as a same-CSV boot-order control). Naive per-nominal-size deltas are themselves partly an artifact of the synthetic session driver's own text length landing a rep's real extend length near a padded-bucket edge (see the pitfall in [`../../tooling/serving-benchmark.md`](../../tooling/serving-benchmark.md)); regrouping every rep by its actual landed bucket, the largest bucket-controlled GPU-time delta anywhere is 1.33 ms, close to or smaller than that same bucket's own A1-vs-A2 control delta, two orders of magnitude below the predicted 12-22 ms bands.
4. **Dispatch path and served M confirmed directly on the live server** (not inferred): the six dense-projection shapes all reach `F.linear` at exactly one of the 13 gemm-pad-m buckets on every call, ruling out both "a non-TunableOp aiter branch serves this shape" and "served M does not match the tuned bucket." What remains is a CUDA-graph-capture interaction: every shape's first-ever invocation happens while `torch.cuda.graph()` capture is active (`BaseRunner.warmup()` never touches dense-GEMM shapes; `PrefillCudaGraphRunner.capture_one_shape()` is the first caller), and an isolated single-GPU reproduction of that exact condition (zero prior process GEMM activity, then a first touch during capture) throws `Hip error: 'operation not permitted when stream is capturing'(900)` at `hipblaslt.cpp:171` every time, while a single prior eager touch of any shape avoids the crash every time. Production does not crash, so something earlier in boot provides enough process warmth to avoid it, but that same warmth does not also cause the tuned kernel to be selected: this job's own GPU-time measurement with the merged CSV loaded and the correct M confirmed still lands inside the same untuned-looking band the boot-order-controlled retest above found. Precisely why production avoids the crash without also picking the tuned algorithm is not pinned down (candidate follow-up: instrument inside PyTorch's own TunableOp C++ path, or vary what runs before capture); not retried a further time without a plan.

This is the fourth refutation of this candidate, each at a more direct level than the last (no-graph client TTFT, breakable-graph client TTFT, breakable-graph per-forward GPU time bucket-controlled, breakable-graph per-forward GPU time with dispatch path and M confirmed). PR uw-syfi/sglang#114 (the merged bucketed table) stays closed, not pursued.

**LM head has its own, separate instance of the aiter tuned-GEMM gap.** It never reaches `UnquantizedLinearMethod.apply()` at all: `should_apply_lm_head_quant_method` is `False` for this model, so `logits_processor.py` calls a bare `torch.matmul` at the raw, unpadded prefill M, generalizing the `mtp_fc` finding above to the single most expensive of the seven originally-tuned shapes (N=62080). Not measured directly (LM head is one launch per forward, not the 60-120 the dense projections get), flagged as a separate, concrete, addressable bug rather than folded into the refutation above.

Scope: rocm, gfx942, aiter bf16 dense-GEMM prefill path (`unquant.py`), TP=4, this model. Status: per-bucket saving verified (measured, reproduces this section's estimate); end-to-end acceptance refuted four times (no-graph TTFT, breakable-graph TTFT, breakable-graph GPU time bucket-controlled, breakable-graph GPU time with dispatch/M confirmed); the capture-time first-touch interaction is the surviving explanation, not fully mechanized; `mtp_fc` and LM head entries correct but inert pending a separate padding-wrapper fix for each. Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified.
