...
Scope: rocm, gfx942, aiter bf16 dense-GEMM prefill path (`unquant.py`), TP=4, this model. Status: per-bucket saving verified (measured, reproduces this section's estimate); end-to-end acceptance refuted twice (once without and once with the prefill graph active); `mtp_fc` entries correct but inert pending a separate padding-wrapper fix. Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified.
