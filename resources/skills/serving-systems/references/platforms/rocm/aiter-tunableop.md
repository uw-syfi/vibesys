...
### The bucketing scheme is accepted: the four prior refutations were one file-level bug
...
That per-bucket saving failed to survive to end-to-end TTFT across four successive jobs (no-graph client TTFT, breakable-graph client TTFT, breakable-graph GPU time bucket-controlled, breakable-graph GPU time with dispatch path and served M confirmed directly on the live server). All four shared one undiagnosed root cause, found by the fourth job:

```
Symptom: kernel names in a served torch-profiler trace never match the
         tuned solutions in the results CSV, at every dense-GEMM shape
         and every M, whether the prefill CUDA graph is active or eager.
         Aggregate per-forward GPU time also never moves, with or
         without the merged table staged.
Cause:   the merged TunableOp CSV's five Validator header lines carried
         CRLF line endings. PyTorch's TunableOp results loader compares
         these header lines by exact string match and discards the
         entire results file, not just the mismatched lines, on any
         failure, silently: no boot failure, one warning per rank,
         easy to miss among boot noise: "results validator check
         failed, Failed validator ROCBLAS_VERSION, Tunable.cpp:366".
Fix:     stage TunableOp CSVs with LF line endings only; add a hard
         check at staging (0 CR bytes on every per-rank file) before
         boot; grep server logs for "validator" after every TunableOp
         change, not just for the load-verify success line.
Scope:   PyTorch TunableOp, any backend or (gfx, cu_num); a hand-merged
         or hand-edited CSV is the likely source of CRLF, not a fresh
         tuning run's own CSV writer. See
         [`../../frameworks/pytorch.md`](../../frameworks/pytorch.md#a-tunableop-results-csv-with-crlf-line-endings-silently-discards-the-whole-table)
         for the portable version of this pitfall.
Status:  verified. sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified.
```

The decisive diagnostic was a kernel-name audit, not aggregate GPU time: a torch-profiler capture with `record_shapes` off, matching each served dense-GEMM launch's HipBLASLt kernel name against the results CSV's own tuned-solution column for that shape and M. Aggregate GPU-time A/B cannot tell "the tuned tile never runs" apart from "the tuned tile runs but does not help"; the kernel-name match can. With the accepted 252-row table (which never contained prefill-M rows) or the CRLF-broken merged table, 345 of 345 dense-GEMM launches matched the untuned baseline kernel name at every shape, at every M, prefill CUDA graph on or off. With the LF-normalized table staged, the inverse held exactly: 345 of 345 matched the tuned kernel name at every M tested. A related pitfall found while chasing this: setting `PYTORCH_TUNABLEOP_RECORD_UNTUNED` together with `PYTORCH_TUNABLEOP_UNTUNED_FILENAME` on this multi-rank boot silently disabled all TunableOp load logging on the served path (caught by the load-verify hard gate, not a silent pass); drop that instrumentation pair for any boot meant to verify a table load.

With the fix understood, the candidate was re-run as a full paired end-to-end acceptance (three jobs: c48, a 16-session cap, and accuracy) and **accepted**:

| Concurrency | pooled p95 TTFT turn2+ | pooled p50 | TPOT (median) |
|:--|--:|--:|--:|
| Uncapped, 48 sessions | 342.1 -> 206.1 ms (-39.7%) | 143.1 -> 119.2 ms (-16.7%) | 11.03 -> 10.07 ms (-8.7%) |
| 16-session cap | 202.6 -> 180.7 ms (-10.8% raw, -16.4% collapse-excluded) | 141.2 -> 118.7 ms (-15.9%) | 8.97 -> 8.82 ms (-1.7%) |

`schedule_bound_fraction` is 1.000 on every rep, both concurrencies: this benchmark is fully queue-bound, so a per-forward GPU-time saving compounds through the queue and the tail improves by more than the median (c48: p50 -16.7%, p90 -20.1%, p95 -39.7%, p99 -58.9%). Accuracy: GSM8K level-1 94.6 percent (473/500), inside the accepted [91.9, 96.3] band; level-2 base-vs-candidate agreement lands inside the lossy-candidate headroom on all three metrics (text-identical, mean token-agreement, mean |delta logprob|). Gates 13/13 on every rep, both sides, all three jobs; 0 CR bytes on every per-rank staged CSV confirmed each time.

This candidate was refuted four times before the CRLF bug was found; none of those four jobs ever observed a tuned-kernel effect, because the table never loaded a single row in any of them. PR uw-syfi/sglang#114 (the merged bucketed table, LF-fixed) is the accepted PR.

**LM head has its own, separate instance of the aiter tuned-GEMM gap, unaffected by the fix above.** It never reaches `UnquantizedLinearMethod.apply()` at all: `should_apply_lm_head_quant_method` is `False` for this model, so `logits_processor.py` calls a bare `torch.matmul` at the raw, unpadded prefill M, generalizing the `mtp_fc` finding above to the single most expensive of the seven originally-tuned shapes (N=62080). Not measured directly (LM head is one launch per forward, not the 60-120 the dense projections get), flagged as a separate, concrete, addressable bug, not folded into the acceptance above.

Scope: rocm, gfx942, aiter bf16 dense-GEMM prefill path (`unquant.py`), TP=4, this model. Status: accepted (paired end to end, three jobs). `mtp_fc` and LM head entries correct but inert pending a separate padding-wrapper fix for each. Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified across three paired jobs (c48, c16, accuracy).
