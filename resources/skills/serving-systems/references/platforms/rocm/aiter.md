...
A nine-point sweep of these two kernels (batch size 2 to 919 tokens) confirms the flat-cost claim directly: combined device time holds at 9.5 to 9.7 us regardless of batch size, pure launch and bookkeeping overhead rather than work that scales with tokens routed. The same sweep also closes the verification path above: aiter's competing CK `moe_sorting` kernel is not a faster drop-in. It is equally flat with batch size (9.9 to 10.0 us device time) but not faster, within about 2.5 percent of the current kernels at the device level and 4 to 12 us per call slower at the host-dispatch level across most batch sizes measured (one exception within noise). Correctness of a drop-in swap is fully confirmed: the two kernels assign the exact same (token, expert) pairs at every batch size tested (they differ only in which physical row within a padded block holds a given token, which does not affect the MoE output), and a paired output check found the two paths bit-identical once a buffer-capacity mismatch in the conversion step was fixed (an unrelated production kernel keys its scaffold-vs-templated dispatch off allocated capacity, not the real block count, so a naively-sliced conversion buffer picks the wrong kernel variant at large batch size and produces a bf16-rounding-level difference, not a routing bug). Predicted effect of the swap on round time is a small regression, not a saving (about 1.5 to 1.8 percent worse at a representative batch size from the host-dispatch numbers; likely negligible either way once the call is CUDA-graph-captured, since graph capture pays dispatch overhead once, not per replay). The two kernels also use different physical index encodings for the identical routing decision (a flat `token*top_k+slot` index here vs. aiter's packed `(slot<<24)|token` index), so integrating one for the other needs an explicit, exact conversion step, not a bit-compatible substitution.

Status: verified (fixed-cost claim reproduced by an independent sweep; the drop-in-replacement question is closed: correctness confirmed, but refuted as a speedup, so not recommended as a swap). Scope: rocm, gfx942, SGLang NEXTN k=3 speculative decode with the fused MXFP4 MoE kernel, this image, TP=4. Stamp: `sglang-v0.5.18-rocm700-mi30x`, 2026-09-12, job-verified.
...
The tuned-GEMM pitfall above explains why the fallback fires; the dispatch is a `functools.lru_cache(maxsize=4096)`-wrapped lookup keyed on `(M, N, K, dtype, otype, bias, scaleAB, bpreshuffle)`, so the whole function body, including the "not found tuned config" log line, runs only on the first call with a given key: the miss line and "first touch of this shape" are the same event, by construction. The fallback it returns is a direct call into `torch.nn.functional.linear`; nothing in PyTorch's own hipBLASLt wrapper caches a per-shape algorithm choice on top of that, so the fallback's own dispatch cost is paid on every call through this path, not specially on the first. The shipped tuned-GEMM CSV for this model targets gfx950 and contributes zero matching rows on gfx942 (see the pitfall above), so this fallback runs for nearly every dense-projection prefill shape here.

A live single-request TTFT sweep isolates the first-touch component from the surrounding fallback tax: the first request to use a brand-new exact M (never seen since the last cache reset) costs about 13 to 20 ms more than a request reusing an already-seen M, at 10 of 12 sampled prompt lengths; at two on-grid lengths the gap was 184 to 205 ms. This tax is real but small, separate from the aiter fallback's own steady per-call cost above and from a much larger first-request-after-cache-flush cost that dwarfs it; see [`../../engines/sglang.md`](../../engines/sglang.md) and [`../../tooling/serving-benchmark.md`](../../tooling/serving-benchmark.md) for that mechanism and the collapse it is not the cause of.
...
A per-rank kernel-level prefill profile at message sizes 2.7 to 7.2 MiB (337 and 919 extend tokens, TP=4) found the collectives bucket 3.7x to 7.1x the size a clean, isolated microbenchmark of the same kernel and message size predicts (production per-launch cost 423 to 537 us against a 76 to 115 us clean median for the identical kernel, size, and iteration count), while every compute-only bucket (MoE, dense GEMM, attention, norm) matched a floor-based prediction to within a few ms. A microbenchmark comparing aiter's custom two-stage all-reduce (`cross_device_reduce_2stage`) against plain RCCL at four message sizes (0.5 to 8.0 MiB) found aiter's implementation at or ahead of RCCL at 3 of 4 sizes, including both production sizes (ahead by 2.4 to 7.8 percent, never behind by more than 1.3 percent), with RCCL showing a consistently higher elementwise mismatch count against an fp64 reference at every size (for example about 8.6 percent of elements over 1 ULP for RCCL against about 0.18 percent for aiter at the larger production size), both implementations inside the accepted bf16-rounding tier. **Swapping the all-reduce implementation at these message sizes is refuted**: the two sit within a few percent of each other, aiter already wins or ties at the sizes that matter, and RCCL is measurably less exact.

The per-rank pattern points elsewhere. At a fixed extend-token count, a rank's own (collectives-time, host-gap-before-launch) pair varies severalfold across ranks in the same forward while the pair's sum stays inside a narrow band (four ranks summing to 120.5 to 127.1 ms of combined collectives-plus-gap time at one token count). This is consistent with `cross_device_reduce_2stage`'s signal-and-spin design (every rank launches, then spins on a signal flag until the other ranks post their own contribution) attributing a rank's wait for its slower peers to its own "collectives" kernel-duration measurement, while a rank that instead arrives late to the call shows the identical wait as idle host-side gap before the launch: whatever inflates this bucket is a property of when each rank arrives at the collective call, not which all-reduce algorithm runs underneath it. Candidate, not confirmed: supported by one job's correlational pattern (a roughly constant per-rank sum), not by a test that deliberately injects or removes arrival skew.

Scope: rocm, gfx942, aiter's `cross_device_reduce_2stage` kernel, TP=4, prefill message sizes 0.5 to 8.0 MiB. Status: all-reduce-implementation swap refuted (measured, isolated microbenchmark); arrival-skew attribution is a candidate (one job's correlational evidence, not an injected-skew test). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified.
