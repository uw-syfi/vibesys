...
**Follow-on: the on-device construction this section calls for was built, and needed far less kernel rewriting than estimated.** A feasibility check (single-GPU microbenchmark, no server) built the on-device chunk-index construction this section's own fix description calls for (`torch.searchsorted` against the already device-only `prepare_chunk_offsets`, no `.tolist()`/`.item()`), plus `NT_max` padding with a per-program early-exit guard, across all four chunked-scan kernels. Contrary to "rewriting all four kernels" above, `chunk_delta_h` needed zero kernel-body changes (its own grid is already sized off `N * H`, not `NT`, and its per-sequence chunk count was already computed on device); the other three needed one early-exit guard line each. Bit-exact on 12 of 12 comparisons (3 request-composition cases, including a continuation with prior SSM state, times 4 token buckets; max abs diff 0.0 on all 36 tensor checks), static time within -2.3 to +0.7 percent of dynamic per layer (static faster on 9 of 12 rows, well inside a 10 percent budget), and one layer's static forward captures under `torch.cuda.graph` and replays bit-exact against a different real input at the same bucket. This refutes only the earlier *cost* estimate, not the mechanism: `causal_conv1d_fn`'s own grid axis and full call-site wiring (a real `NT_max` bound from production batch-composition data, all 45 layers, a paired TTFT job) were left to a follow-up integration attempt (below), estimated at 3-4 remaining days rather than the several days a kernel rewrite alone would have cost.

Scope: gfx942 MI300A, single-GPU microbenchmark with random weights at real TP4 per-rank shard shapes; not yet wired into any call site or measured on a live server. Status: verified (bit-exact, 12/12 comparisons; capture and replay bit-exact). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-14, microbench-verified.

## Rejected: wiring the static-shape kernels into the breakable prefill CUDA graph corrupts output under concurrency

An integration attempt wired the feasibility check's static kernels behind a new switch (`SGLANG_GDN_STATIC_PREFILL`, default off, byte-identical to the accepted head when off) so the whole 45-layer GDN stack for one prefill bucket runs inside a single captured graph segment instead of breaking 45 times. Two capture-incompatibility bugs, unrelated to the static-kernel design itself, were found and fixed along the way:

- **A wrapper attribute-resolution bug.** This hybrid (attention plus GDN) model's own `get_attn_backend()` returns a `HybridLinearAttnBackend` wrapper, not the GDN backend directly; the real GDN backend instance lives at `.linear_attn_backend`. Code that treats the wrapper as the GDN backend itself crashes during capture with an `AttributeError`.
- **A `.new_tensor()` capture-incompatibility.** A shared helper (`prepare_chunk_offsets`) builds a small constant tensor with `cu_seqlens.new_tensor([0])`, a host-to-device copy that is not capturable mid-capture (`HIP error: operation not permitted when stream is capturing`). Fix: a capture-safe variant built with `torch.zeros` instead, used only by the static-path call sites; the shared dynamic-path function is untouched.

With those fixed, capture succeeded on all 4 TP ranks at all 13 buckets, and per-forward graph breaks fell from 60 (45 GDN plus 15 attention) to 15, one better than the 16 predicted: the one remaining GDN setup break is the first operation in the forward, so it does not split an already-open segment. Two results then diverged from the design's own predictions:

1. **Forward-wall saving was 2.1 to 6.9 ms, not the predicted 8 to 13 ms.** The static kernels always run at a fixed, worst-case padded chunk count (`nt_max = ceil(bucket/64) + (GDN_STATIC_R_MAX - 1)`, since a captured segment's launches cannot vary shape per replay), while the dynamic kernels they replace size to the real, usually-smaller chunk count. On this campaign's own real traffic (94.6 percent of turn-2+ batches are a single request), the static path does meaningfully more GDN compute than a typical prefill needs, eating roughly half the break-removal saving: the gap-time (idle-time-from-breaks) component of the prediction matched almost exactly (8.84 to 13.70 ms of gap time actually removed versus a 7.8 to 12.6 ms estimate), but a 3.6 to 7.5 ms *increase* in GPU-busy time at every size, not predicted, canceled roughly half of it.
2. **The accuracy gate failed at concurrency 64.** GSM8K level-1 accuracy 62.4 percent (312/500) versus base's 94.0 percent (470/500), 31.6 points below base and far outside the accepted tolerance band; level-2 base-versus-candidate agreement showed near-zero token agreement (0.0127 against a required 0.15) and zero text-identical outputs. Direct inspection of the generated text showed output unrelated to the input prompt (an unrelated biology question, tutorial code, a different math problem, in response to GSM8K arithmetic questions), consistent with cross-request state corruption, not a numerical rounding difference. Paired TTFT/TPOT acceptance jobs were not run once this was found: measuring latency for a change known to corrupt output would be misleading.

**Best-supported cause, not confirmed by direct instrumentation.** The per-forward metadata refresh (padding a batch's `query_start_loc` and related fields into the fixed-size static buffers) is gated by a raw `id(forward_batch)` comparison, not a monotonic counter or generation token. CPython reuses a freed object's address, so a newly allocated `ForwardBatch` can alias a previously freed one's address under high request churn, making `id(new) == id(old)` true for a genuinely new forward and silently skipping the refresh, including its own overflow guard. This generalizes beyond this design: gating a once-per-event refresh on an object's `id()` is unsafe whenever the object's lifetime is not guaranteed to outlive the check, on any engine or backend, not only this one; use a monotonic counter or generation token instead. Server logs from the concurrency-64 run show real prefill batches of 11 to 12 new sequences running through the captured graph, above the design's own `GDN_STATIC_R_MAX = 8` bound, with zero matching `RuntimeError` lines: the overflow guard that should have caught this never fired, consistent with the refresh being skipped rather than the bound simply being too low.

A second, related lesson: **a static bound derived from per-session batch composition does not describe concurrency-level batch composition.** `GDN_STATIC_R_MAX = 8` was set from this campaign's own turn-2+ per-session data (94.6 percent single-request, a 4x margin over the observed per-session maximum), which describes how many turns of *one* conversation land in one prefill batch, not how many different, unrelated sessions' prompts get batched together at high overall concurrency; real batches reached 11-12 sequences at concurrency 64. A static bound for a capture-time buffer needs deriving from the batch sizes the capture will actually see in production, not from a narrower, session-scoped statistic.

**Remaining lever for TTFT, with a reduced expected saving.** GDN prefill capture is rejected as designed, not merely unfinished: even fixed, the worst-case-padding cost already roughly halves the plausible saving versus the original break-removal estimate, and the correctness bug found here needs a different staleness mechanism (not just a larger `R_MAX`) before revisiting is safe. If revisited, `GDN_STATIC_R_MAX` needs re-deriving from concurrency-level batch composition, and the achievable forward-wall saving should be expected nearer 2-7 ms per forward than the original 8-13 ms estimate.

Scope: rocm, gfx942, this fork's chunked delta-rule kernels and hybrid-attention backend wrapper. Status: rejected (correctness regression reproduced on real hardware at concurrency 64; the wiring, capture, and break-count-reduction mechanism is verified working, but the design as a whole does not ship). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-14, job-verified.
