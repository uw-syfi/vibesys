...
## Prefill M: wider tiles (64/128 rows) analytically closed; no aiter/CK grouped-GEMM alternative on gfx942

Follow-up design study (analytical only, no build) to the SUBM=2 refutations above, asking whether a once-per-tile decode design at a wider row count (widen the MFMA tile past 32 rows, decode each expert's weights once per tile instead of once per 16-row block) is a different mechanism from the already-refuted SUBM=2 kernel, and whether an existing library kernel sidesteps the whole question.

**Not a different mechanism.** The dispatched kernel already issues MFMA (`gemm16x16_fp4_accumulate` / `gemm_wide_tpl`, both calling `__builtin_amdgcn_mfma_f32_16x16x16bf16_1k`) with `f32x4` accumulators held live across the whole K-loop; `BLOCK_M=16` is stated in-source as matching the MFMA tile. A 64-row or 128-row tile is the identical accumulator-liveness mechanism as SUBM=2, carried further, not a new one.

Register-file accounting (calibrated against three of this file's own measured `amdhsa_metadata` points, exact fit to a flat 512-register/wave budget at occupancy 1) shows only the already-built 32-row tile has headroom at any occupancy: a 64x128 tile needs the full occupancy-2 AGPR budget with zero margin, and 64x256 or 128x128 tiles exceed it outright (predicted VGPR 240-368 against a 128-256 budget). A double-buffered K-slice large enough to decode once and reuse across row-blocks (`BK=32, BN=256`) fits LDS with headroom on paper, but reusing one decoded K-step across `R` row-blocks requires all `R` blocks' accumulators to stay live simultaneously through the whole K-walk, which is exactly the register pressure that already sank SUBM=2 twice (see above) and gets strictly worse as R grows from 2 to 3-4.

**Verdict: refuted on paper, no build.** The 2x-plus improvement the bandwidth-bound arithmetic predicts (0.231 ms/layer idealized versus 0.645-1.333 ms measured) is real as a bandwidth argument, but reaching it requires solving the same accumulator-liveness dilemma the campaign has already built and measured four independent attempts at (SUBM=2 doubled-epilogue on two tile sizes, single-epilogue, and atomic-reduction), all net slower with a PMC-confirmed mechanism (epilogue synchronization and LDS traffic exceeding the decode-reuse saving) that gets worse, not better, as register pressure is specifically reduced. Extrapolating that mechanism to R=3-4 predicts the same or a worse outcome. The MoE prefill kernel lever is closed for this design; the only cheap remaining probe is a compile-only SUBM=3 register/spill check, not attempted.

**No aiter or CK MXFP4 grouped-GEMM kernel exists for gfx942 either**, closing off a library alternative to a from-scratch redesign: every MXFP4-weight kernel in the bundled aiter build is behind an `is_fp4_avail` check scoped to `gfx950`/`gfx1250` (aiter `d9e5ef7ce0`), aiter's own CK codegen wraps every generated instance whose B dtype contains FP4 in `#ifndef __gfx942__`, and the CK MX-GEMM grouped-GEMM pipeline itself requires gfx950's native support. See [`aiter.md`](aiter.md) and [`../../algorithms/quantization-schemes.md`](../../algorithms/quantization-schemes.md) for the gfx942/gfx950 MXFP4 split this confirms again at the grouped-GEMM level.

Scope: rocm, gfx942, MI300A TP=4, this fork's fused MXFP4 MoE kernel, prefill M 192-1024. Status: refuted (analytical: register/LDS accounting from kernel source, plus a mechanism generalized from four already-measured builds; no new build run for this study). What would change this: a compile-only SUBM=3 register/spill check, or a gfx950 MXFP4 grouped-GEMM kernel becoming available and portable back. Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified (kernel source read plus arithmetic derived from prior measured builds).
