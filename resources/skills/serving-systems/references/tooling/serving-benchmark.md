...
Fix, measured: run one warmup rep, then flush the cache (e.g. `POST /flush_cache` while idle) before each measured rep. Over three reps on one node, this cut p95 TTFT run-to-run spread from 39 percent (unflushed) to 8-18 percent, and held TPOT spread at 0.2-0.3 percent; a same-node paired A/B run under this protocol then resolved a real difference between two builds (TTFT -4.85 percent, TPOT +1.58 percent), both passing the accuracy gate 13/13. Rep 1 without a flush is the number comparable to a fresh-server evaluation; treat reps taken without an intervening flush as one contaminated sample, not independent repetitions.
