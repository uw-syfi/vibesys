...
- **Neither a trace's own host-clock range annotation nor a kernel-class burst detector finds true per-layer boundaries in an async-dispatched trace.** A range annotation on the CPU host thread ends before the GPU has actually finished the layers it dispatched (the host races ahead of the async queue; confirmed case: up to 15 of a trace's own 60 per-layer kernels for the final layers landed after the CPU-side window's own end). A burst detector that groups consecutive same-class GPU kernels also fails when one layer's mixer interleaves ~10 named kernels with ~20-30 small elementwise "bookkeeping" kernels from the same eager call, fragmenting one real layer into several detected pieces. What works: find a kernel that is emitted exactly N times per layer by a structural property of the model (for example a fused residual-add-plus-norm kernel called exactly twice per transformer-style layer, once after each of a layer's two sublayers), confirm the total count equals N times the known layer count, and use every Nth occurrence as an exact layer boundary on the GPU-side timeline. Scope: any layer-structured model traced with an instrumenting profiler under async kernel dispatch, engine- and backend-agnostic. Status: verified (mechanism confirmed by the host/GPU-window discrepancy and the burst-detector fragmentation, both reproduced before the fix; the marker-kernel count matched the known layer count exactly in three separate captures). Stamp: sglang-v0.5.18-rocm700-mi30x, 2026-09-13, job-verified.
