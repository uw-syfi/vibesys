...
* **attention** (`nl.ndarray`) – Input tensor in HBM, typically the scores output from an attention block. Shape: `[B, N, D, S]`, where `B` is batch size, `N` is number of heads, `D` is head dimension, and `S` is sequence length. Indexing: `[b, n, d, s]`.

* **weight** (`nl.ndarray`) – Weight tensor in HBM. Shape: `[N*D, H]`, where `H` is hidden dimension size. Indexing: `[n * D + d, h]`.

* **bias** (`nl.ndarray`, optional) – Optional bias tensor in HBM. Shape: `[1, H]`. Indexing: `[1, h]`.

Returns:
Output tensor in HBM. Shape: `[B, S, H]`. Indexing: `[b, s, h]`.

Return type:
`nl.ndarray`

**Data Types**:This kernel supports `nl.float32`, `nl.float16` and `nl.bfloat16` data types.
