...
def test_permission_failure_and_child_target_are_structured(tmp_path: Path):
    ...
    with (
        patch("vibesys.macos_cpu_profiler.subprocess.Popen", return_value=process),
        patch("vibesys.macos_cpu_profiler._descendants", return_value=[456]),
        patch("vibesys.macos_cpu_profiler.time.sleep"),
        patch(
            "vibesys.macos_cpu_profiler.subprocess.run",
...
def test_sample_kills_launcher_when_graceful_wait_times_out(tmp_path: Path):
    ...
    with (
        patch("vibesys.macos_cpu_profiler.subprocess.Popen", return_value=process),
        patch("vibesys.macos_cpu_profiler._descendants", return_value=[]),
        patch("vibesys.macos_cpu_profiler.time.sleep"),
        patch("vibesys.macos_cpu_profiler.subprocess.run", return_value=Result()),
    ):
        result = collect(["./benchmark"], tmp_path, capability=capability)
    assert result.status == "ok"
