...
class TestAgentLabel:
    ...
    def test_snake_case_becomes_title_case(self, kind: str, expected: str) -> None:
...
class TestDiscoverSkillDirs:
    ...
    def test_tier_organized_layout(self, tmp_path: Path) -> None:
        deep = _skill(tmp_path, "tier1/nested")

        assert discover_skill_dirs(tmp_path) == [deep]

    def test_a_root_with_no_skills(self, tmp_path: Path) -> None:
...
class TestMaterializeSkills:
    def test_copies_into_every_cli_discovery_path(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        _skill(src, "alpha")
        ws = tmp_path / "ws"
        ws.mkdir()
    ...
    def test_no_sources_is_a_noop(self, tmp_path: Path) -> None:
        ws = tmp_path / "ws"
        ws.mkdir()
    ...
    def test_sources_without_skills_create_nothing(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        src.mkdir()
    ...
    def test_re_materializing_replaces_stale_content(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        _skill(src, "alpha", "# v1\n")
        ws = tmp_path / "ws"
        ws.mkdir()
    ...
    def test_source_already_at_workspace_root_is_not_replaced(self, tmp_path: Path) -> None:
        ws = tmp_path / "ws"
    ...
    def test_later_source_wins_on_a_name_collision(self, tmp_path: Path) -> None:
        first = tmp_path / "a"
        second = tmp_path / "b"
    ...
    def test_a_copy_failure_is_logged_not_raised(
    ...
    def test_a_copy_failure_without_a_log_is_swallowed(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        src = tmp_path / "src"
        _skill(src, "alpha")
    ...
    def test_a_stale_symlink_destination_is_replaced(self, tmp_path: Path) -> None:
        src = tmp_path / "src"
        _skill(src, "alpha")
        ws = tmp_path / "ws"
...
class TestPlatformPruning:
    ...
    def test_prunes_foreign_platform_dirs(self, tmp_path: Path, selected: ComputeBackend) -> None:
    ...
    def test_keeps_portable_tiers_and_same_named_dirs(self, tmp_path: Path) -> None:
    ...
    def test_without_a_backend_every_platform_is_kept(self, tmp_path: Path) -> None:
...
class TestBuildSchemaHint:
    def test_names_the_model_and_embeds_its_schema(self) -> None:
