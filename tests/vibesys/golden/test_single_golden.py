"""Golden snapshots for the single-agent strategy: prompts, board files, events.

Drives ``SingleAgentOrchestrator.run(ctx)`` end-to-end (real workspace, git
tracking, progress board, event journal) against a scripted
:class:`FakeAgentClient`, the same integration pattern
``tests/vibesys/golden/test_multi_golden.py`` uses for multi.
No real agent CLI, sandbox, or subprocess ever runs.

Single-agent has no separate pre-round profiling decision and no separate
judge role: one designer (``orchestrator``) produces the round's
``OrchestratorPlan``, and one worker (``implementer``) both implements and
self-judges in a single combined turn that returns
...
- ``pass``: one round, plan -> combined turn PASS. ``backend_name="stub"``
  short-circuits framework gates (single skips them entirely for a stub
  backend), keeping this scenario's snapshot free of gate noise.
- ``retry_then_pass``: the combined turn FAILs attempt 1, feedback carries
  into attempt 2's prompt, the combined turn PASSes attempt 2.
...
def _plan() -> OrchestratorPlan:
    return OrchestratorPlan(
        ...
        pass_criteria="throughput improves without regressing accuracy",  # noqa: S106  # LW-040020 [S106]; the argument is a fixture literal, not a credential.
...
def _scripted_accuracy_gate(*, passed: bool):  # noqa: ANN202  # LW-040021 [ANN202]; the helper is private to this test module and its return type is the local closure type.
    ...
    def _run(ctx, *, process_id, timeout_seconds=None, execution_command=None, round_label=None):  # noqa: ANN001, ARG001, ANN202  # LW-040022 [ANN001, ANN202, ARG001]; this scripted double mirrors a production signature whose parameters are not annotated here. The helper is private to this test module and its return type is the local closure type. This scripted double accepts the production keyword arguments and ignores the ones it does not need.
...
# TODO(stack PR 05): restore test_timeout_scenario_golden (single/turns.py's  # noqa: TD003, FIX002  # LW-040023 [FIX002, TD003]; the placeholder marks work owned by a later change and has no issue yet.
