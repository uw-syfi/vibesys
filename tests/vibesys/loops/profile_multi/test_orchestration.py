...
def _descriptor(**updates: object) -> OrchestrationDescriptor:
    return descriptor_from_options(
        _options(**updates), orchestration_id="profile-guided-multi-agent"
    )
...
def test_options_reject_wrong_strategy_and_incompatible_settings() -> None:
    ...
    orchestrator = ProfileGuidedMultiAgentOrchestrator(descriptor)
    assert orchestrator.setup.state_namespace == "profile_multi"
...
def test_projector_uses_profile_multi_namespace_and_ignores_other_commits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = AgentRunState()
    load = MagicMock(return_value=state)
    projected = MagicMock(return_value="view")
    monkeypatch.setattr("vibesys.loops.profile_multi.orchestration.load_agent_run_state", load)
    monkeypatch.setattr("vibesys.loops.profile_multi.orchestration.project_run_view", projected)
    projector = ProfileMultiProjector()
    project = MagicMock()

    assert projector.view(project, "run-1", status=RunStatus.ACTIVE, loop="profile") == "view"
    load.assert_called_once_with(project, "run-1", namespace="profile_multi")
    assert projector.project_committed("multi", state, run_id="run-1") is None
    assert projector.project_committed("profile_multi", state, run_id="run-1") == "view"
    assert projected.call_args.kwargs["loop"] == "profile-guided-multi-agent"
...
def test_run_orders_attempts_gates_commit_and_cleanup(monkeypatch: pytest.MonkeyPatch) -> None:
    session = _Session()
    monkeypatch.setattr(ProfileMultiSession, "open", AsyncMock(return_value=session))
    boundary = AsyncMock()
    ctx = cast("RunContext", SimpleNamespace(control=SimpleNamespace(boundary=boundary)))

    assert asyncio.run(ProfileGuidedMultiAgentOrchestrator(_descriptor()).run(ctx))
...
def test_run_closes_session_when_review_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="judge failed"):
        asyncio.run(ProfileGuidedMultiAgentOrchestrator(_descriptor()).run(ctx))
