"""Profile multi role ordering, isolation, and durable role evidence."""

# ruff: noqa: SLF001  # LW-030012; Read-only guards and role turns are the policy under test.
...
def _turns(tmp_path: Path) -> ProfileMultiTurns:
...
def test_implementer_marks_paid_turn_after_prompt_and_judge_applies_pareto_guard(
    ...
    monkeypatch.setattr(
        "vibesys.loops.profile_multi.turns.issue_board.write_implementer_start_marker", marker
    )
