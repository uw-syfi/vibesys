"""Profile guided multi hypothesis transitions from completed round evidence."""
...
def test_completed_round_advances_profile_cursor_and_handoff(  # noqa: PLR0913
    ...
    engine = HypothesisEngine.create(
        AgentRunState(), config=ProfileGuidedInput(command=("profile",))
    ).start(plan, started_round=1, parent_commit="a" * 40)
    state = engine.state
    ...
    record = RoundRecord(
        ...
        perf_delta_pct=5.0 if passed else None,
    )
