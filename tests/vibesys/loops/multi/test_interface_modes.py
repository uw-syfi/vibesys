...
def _effective_profiler_definition(
    ...
    return turns._profiler()  # noqa: SLF001  # validate the strategy's profiler guard


def _profiler_prompt_template(
    kind: ProfilerKind, *, supports_torch_profiler: bool = False
) -> str | None:
    definition = _effective_profiler_definition(
        kind, supports_torch_profiler=supports_torch_profiler
    )
    return definition.prompt_template if definition is not None else None
...
def test_loop_rejects_unknown_interface():  # noqa: ANN201  # tracked: #288
...
def test_standalone_profiler_none_has_no_prompt_template():  # noqa: ANN201  # tracked: #288
    assert _profiler_prompt_template(ProfilerKind.NONE) is None
