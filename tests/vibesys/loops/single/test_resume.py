"""Resume-after-crash for the single-agent strategy, driven through the
real ``SingleAgentOrchestrator`` (``tests.vibesys.loops._support``, which
uses ``run_orchestration``'s ``agent_client_factory``/``backend_factory``
seams with ``FakeAgentClient``/``FakeComputeBackend``, never
``unittest.mock.patch`` on a collaborator).

The clean pass / retry / gate scenarios are golden-snapshotted in
``tests/vibesys/golden/test_single_golden.py``. This module covers what
those don't: a crash mid-round resumes without repeating the orchestrator's
planning turn, and reaches the same final hypothesis state an uninterrupted
run with the same script would.
"""
...
_ORCHESTRATION_ID = "single-agent"
...
def _descriptor(**overrides: object) -> OrchestrationDescriptor:
    return descriptor_from_options(_options(**overrides), orchestration_id=_ORCHESTRATION_ID)
...
def _plan() -> OrchestratorPlan:
    return OrchestratorPlan(
        ...
        pass_criteria="throughput improves without regressing accuracy",  # noqa: S106  # LW-040146 [S106]; the argument is a fixture literal, not a credential.
        reasoning="scripted",
    )


def _combined(verdict: Verdict = Verdict.PASS) -> SingleAgentRoundResponse:
...
def _final_state(run: AgentRun) -> HypothesisState | None:
    ...
    return load_hypothesis_state(project, run.run_id, namespace="single")


def test_resumes_next_attempt_without_repeating_orchestrator(tmp_path: Path) -> None:
    """A crash mid-combined-turn resumes without re-running the
    orchestrator's planning turn: the committed attempt is the
    crash-safety boundary, so resume starts a fresh attempt.
    """
    crasher = FakeAgentClient(backend_name="stub")
    crasher.enqueue("orchestrator", _plan())
    ...
    crashed = run_agent_loop_expect_crash(
        ...
        _descriptor(),
        _CrashError,
        exp_name="resume-single",
    )

    resumer = FakeAgentClient(backend_name="stub")
    resumer.enqueue("implementer", _combined())
    resumed = run_agent_loop(
        tmp_path, resumer, SingleAgentOrchestrator, _descriptor(), resume_from=crashed
    )
...
@given(crash_role=st.sampled_from(["orchestrator", "implementer"]))
...
def test_resume_reaches_same_final_state_as_uninterrupted_run(
    ...
    """Whichever turn a run crashes mid-way through (planning or the
    combined implement+self-judge turn), resuming it against the same
    script reaches the same final round count and verdict an uninterrupted
    run of that script would.
    """
    baseline_dir = tmp_path_factory.mktemp("baseline")
    baseline_client = FakeAgentClient(backend_name="stub")
    baseline_client.enqueue("orchestrator", _plan())
    baseline_client.enqueue("implementer", _combined())
    baseline = run_agent_loop(
        baseline_dir, baseline_client, SingleAgentOrchestrator, _descriptor(), exp_name="baseline"
    )
    ...
    crasher = FakeAgentClient(backend_name="stub")
    if crash_role == "orchestrator":
        crasher.fail("orchestrator", _CrashError())
    else:
        crasher.enqueue("orchestrator", _plan())
        crasher.fail("implementer", _CrashError())
    crashed = run_agent_loop_expect_crash(
        crash_dir, crasher, SingleAgentOrchestrator, _descriptor(), _CrashError, exp_name="crashed"
    )

    resumer = FakeAgentClient(backend_name="stub")
    resumer.enqueue("orchestrator", _plan())
    resumer.enqueue("implementer", _combined())
    resumed = run_agent_loop(
        crash_dir, resumer, SingleAgentOrchestrator, _descriptor(), resume_from=crashed
    )
