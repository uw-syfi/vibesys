...
def test_docker_environment_mounts_extra_profiler_support_dirs(tmp_path: Path) -> None:
...
def test_docker_environment_ignores_profiler_support_extra_without_a_primary_profiler(
    tmp_path: Path,
) -> None:
    backend = FakeBackend()
    env = build_run_environment(RunEnvironmentSpec("docker"))
