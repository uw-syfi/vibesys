...
def test_kv_store_domain_pack_language_constraint_follows_interface():
    ...
    assert "Implementation language is Python" in inproc
    assert "Implementation language is Python" not in svc
    assert "yours to choose" in svc
