"""Tests for vibe_sys.loops.agent — orchestrator-driven build loop."""
...
from vibe_sys.loops.agent import issue_board
...
def _invoke_orchestrate(tmp_path, ref_file, runner, **kwargs):
    ...
    with (
        ...
        patch(
            "vibe_sys.loops.agent.loop._run_framework_accuracy_gate",
...
def test_framework_accuracy_gate_runs_manifest_command_and_records_pass(tmp_path):
    from vibe_sys.loops.agent.loop import _run_framework_accuracy_gate
...
def test_framework_benchmark_extracts_declared_metric(tmp_path):
    from vibe_sys.input_manifest import BenchmarkResult
    from vibe_sys.loops.agent.loop import (
...
def test_loop_generic_auto_profiler_resolves_to_macos_cpu(tmp_path, ref_file):
    ...
    with patch("vibe_sys.profilers.platform.system", return_value="Darwin"):
        result = _invoke_orchestrate(
            tmp_path,
            ref_file,
            runner,
...
def test_cli_loads_objective_md_from_ref_parent(tmp_path):
    from vibe_sys.cli import _load_objective
...
def test_cli_rejects_modal_with_nsys_profiler(tmp_path, ref_file):
    ...
    from vibe_sys.cli import _build_agent_parser, _validate_agent
...
def _record(round_number: int, perf: float | None, unit: str = "tok/s"):
    ...
    from vibe_sys.loops.agent.loop import _RoundRecord
...
def test_detect_plateau_returns_none_when_too_few_rounds():
    from vibe_sys.loops.agent.loop import _detect_plateau
