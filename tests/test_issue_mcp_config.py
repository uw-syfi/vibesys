...
``libs/agent_cli/``. These tests verify that the per-phase policy params
...
def test_build_perf_eval_spec_sorts_allowed_types_alphabetically():
    ...
    args = spec.args
    assert args[0:2] == ["-m", "vibeserve_agent.loops.plain.mcp_server"]
...
def test_build_spec_uses_provided_store_relpath():
    ...
    assert spec.args[spec.args.index("custom/path/issues.json") - 1] == "vibeserve_agent.loops.plain.mcp_server"
