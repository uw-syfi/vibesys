...
UNORDERED_MAP_ACCURACY_SETTINGS = {
    "unordered-map-swmr": ("32", "100"),
    "unordered-map-mw": ("24", "100"),
}

UNORDERED_MAP_CONTRACT = "linearizable unordered map"
...
def _copy_input_bundle(source: Path, target: Path) -> None:
    shutil.copytree(
        ...
        target,
        dirs_exist_ok=True,
        ignore=shutil.ignore_patterns(".venv", "unordered-map-candidate.so", "target"),
    )


def _materialize_unordered_map_input(
    project_root: Path,
    input_name: str,
    workspace: Path,
) -> Path:
...
def unordered_map_native_runner(tmp_path_factory) -> Iterator[Path]:  # noqa: ANN001  # tracked: #288
    ...
    environment.setenv("VIBESYS_UNORDERED_MAP_NATIVE_RUNNER", str(runner))
    try:
        yield runner
    finally:
        environment.undo()
...
def test_unordered_map_manifests_invoke_go_evaluator_directly():  # noqa: ANN201  # tracked: #288
    ...
    for input_name, scenario in UNORDERED_MAP_INPUTS.items():
        ...
        operations, trials = UNORDERED_MAP_ACCURACY_SETTINGS[input_name]
        expected_suffixes = {
            "accuracy": [
                "run",
                ".",
        ...
        assert manifest["evaluator"] == {"source": "../../evaluators/unordered-map"}
        for section, expected_suffix in expected_suffixes.items():
            command = manifest[section]["command"]
            assert command[:3] == ["go", "-C", "_evaluator/unordered-map"]
    ...
    evaluator = root.parents[0] / "evaluators" / "unordered-map"
...
def test_unordered_map_inputs_use_shared_editable_rust_starter():  # noqa: ANN201  # tracked: #288
    ...
    for input_name in UNORDERED_MAP_INPUTS:
...
def test_starter_make_honors_cargo_target_dir(tmp_path):  # noqa: ANN001, ANN201  # tracked: #288
    ...
    assert (build_dir / "unordered-map-candidate.so").is_file()
    built = cargo_target / "release"
    assert (built / "libunordered_map_candidate.so").is_file() or (
        built / "libunordered_map_candidate.dylib"
    ).is_file()
...
def test_unordered_map_benchmark_pins_linux_workers(tmp_path):  # noqa: ANN001, ANN201  # tracked: #288
    ...
    source = (
        Path(__file__).parents[2] / "examples" / "evaluators" / "unordered-map" / "native_runner"
    )
...
def test_materialized_rust_starter_passes_accuracy(  # noqa: ANN201  # tracked: #288
    ...
    _materialize_unordered_map_input(project_root, input_name, workspace)

    candidate = workspace / "unordered-map-candidate.so"
    shutil.copy2(compiled_unordered_map_candidate, candidate)
    assert candidate.is_file()
    ...
    assert f"PASS - {scenario} linearizable unordered map" in completed.stdout
...
def test_materialized_manifest_commands_run_go_evaluator_directly(tmp_path):  # noqa: ANN001, ANN201  # tracked: #288
    ...
    project_root = Path(__file__).parents[2]
    workspace = tmp_path / "workspace"
    input_dir = _materialize_unordered_map_input(
        project_root,
        "unordered-map-swmr",
        workspace,
    )
...
def test_unordered_map_tbb_baseline_uses_concurrent_hash_map():  # noqa: ANN201  # tracked: #288
    ...
    assert "#include <oneapi/tbb/concurrent_hash_map.h>" in adapter
    assert "tbb::concurrent_hash_map" in adapter
    assert "map_type::accessor" in adapter
...
def built_unordered_map_tbb(tmp_path_factory) -> Path:  # noqa: ANN001  # tracked: #288
    ...
    if not _tbb_is_available():
        pytest.skip("oneTBB (libtbb-dev) is required by the TBB unordered-map baseline")
...
def test_unordered_map_tbb_baseline_builds_and_passes_accuracy(  # noqa: ANN201  # tracked: #288
    ...
    assert f"PASS - {scenario} {UNORDERED_MAP_CONTRACT}" in completed.stdout
