...
def test_launch_routes_help_to_the_canonical_headless_parser():
    with patch("vibe_sys.launcher.subprocess.call", return_value=0) as call:
...
def test_main_exits_with_launcher_status():
    with (
        patch("vibe_sys.launcher.sys.argv", ["vibe-sys", "--example"]),
        patch("vibe_sys.launcher.launch", return_value=7) as launch_run,
...
def test_monitor_does_not_leak_backend_output_after_frontend_attaches(tmp_path: Path):
    ...
    with patch("vibe_sys.launcher._report_backend_failure") as report:
...
def test_monitor_gives_backend_time_to_finish_after_frontend(tmp_path: Path):
    ...
    with (
        patch("vibe_sys.launcher._terminate_backend") as terminate,
        patch("vibe_sys.launcher._report_backend_failure") as report,
    ):
        result = _monitor(frontend, backend, tmp_path / "backend.log")
...
def test_terminate_backend_escalates_from_process_group_signal():
    ...
    with patch("vibe_sys.launcher.os.killpg") as killpg:
...
def test_terminate_backend_tolerates_a_missing_process_group():
    ...
    with patch("vibe_sys.launcher.os.killpg", side_effect=ProcessLookupError):
        _terminate_backend(backend)
