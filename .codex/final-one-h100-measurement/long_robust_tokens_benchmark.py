...
def prompts(prompt_len: int, pool_size: int) -> list[str]:
    ...
    shared_prefix = " ".join(words[word_idx % len(words)] for word_idx in range(prompt_len))
    return [
        (
            f"{shared_prefix}\n\n"
            f"Request {prompt_idx:03d}: answer with one concise sentence."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Robust non-streaming long-prompt token benchmark."
    )
    ...
    parser.add_argument("--prompt-len", type=positive_int, default=3000)
