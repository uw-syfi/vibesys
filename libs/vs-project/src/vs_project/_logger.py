...
def _default_emit(text: str, log_file: TextIO) -> None:
    """Write *text* straight to the log file; the standalone ``lprint`` behavior."""
    log_file.write(text + "\n")
    log_file.flush()
...
class RunLogger:
    ...
    ``emit`` is the implementation behind ``lprint``: it receives the printed
    text and the logger's current writer. The default just writes the text to
    the log file, which is all this leaf library can do on its own. A caller
    that also wants ``lprint`` to publish a diagnostic event on its own
    process-wide event sink (as VibeSys core's loops do) passes that
    behavior in explicitly; this class has no dependency on any particular
    event system.
    """
