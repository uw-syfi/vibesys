...
func TestCloseTimesOutStuckDestructor(t *testing.T) {
	workspace := compileCandidateFixtureWithDefines(t, "VSST_TEST_HANG_ON_DESTROY")
	...
	if _, err := session.invoke(0, request{operation: operationEnqueue, value: 1}); err != nil {
		_ = session.close()
		t.Fatal(err)
	}
...
func TestAccuracyTimesOutStuckCandidateOperation(t *testing.T) {
	workspace := compileCandidateFixtureWithDefines(t, "VSST_TEST_HANG_CAPACITY_ONE")
...
func compileCandidateFixture(t *testing.T, retainInput bool) string {
	defines := []string{}
	if retainInput {
		defines = append(defines, "VSST_TEST_RETAIN_INPUT")
	}
...
func compileCandidateFixtureWithDefines(t *testing.T, defines ...string) string {
	...
	workspace := t.TempDir()
	library := filepath.Join(workspace, "stack-candidate.so")
...
func TestAccuracyRejectsCandidateThatOnlySupportsMaximumLength(t *testing.T) {
	workspace := compileCandidateFixtureWithDefines(t, "VSST_TEST_FIXED_LENGTH_ONLY")
	err := runAccuracy(accuracyConfig{
		candidateConfig: candidateConfig{
			workspace: workspace,
			candidate: "stack-candidate.so",
...
func TestAccuracyRejectsCandidateThatRetainsEnqueueInput(t *testing.T) {
	...
	if err == nil {
		t.Fatal("candidate that retained push input passed copying ABI checks")
	}
}
