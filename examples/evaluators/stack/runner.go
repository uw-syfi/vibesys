...
func nativeRunnerPath() (string, error) {
	runnerOnce.Do(func() {
		if configured := os.Getenv("VIBESYS_STACK_NATIVE_RUNNER"); configured != "" {
		...
		target := filepath.Join(
			os.TempDir(),
			fmt.Sprintf("vibesys-stack-native-%x", digest[:8]),
		)
		...
		runnerPath = filepath.Join(target, "release", "vibesys-stack-native-runner")
		runnerErr = validateRunnerExecutable(runnerPath)
	})
