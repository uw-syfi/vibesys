...
func selectedScenarios(value string) ([]scenario, error) {
	if value == "all" {
		return []scenario{scenarioSPSC, scenarioMPSC, scenarioSPMC, scenarioMPMC}, nil
	}
...
func runCheckCommand(args []string) error {
	...
	scenarioName := flags.String("scenario", "spsc", "Stack scenario: spsc, mpsc, spmc, mpmc, or all")
	capacity := flags.Uint64("capacity", 1024, "Bounded stack capacity")
	valueSize := flags.Uint64("value-size", 8, "Copied stack value size in bytes")
	...
	consumers := flags.Int("consumers", 4, "Consumer count for configurable scenarios")
