...
func TestCorrectnessRequestContainsCopiedPayload(t *testing.T) {
	...
	if got := binary.LittleEndian.Uint64(header[8:]); got != 0 {
		t.Fatalf("reserved header bytes = %d, want 0", got)
	}
...
func TestNativeReferenceWorkerRoundTrip(t *testing.T) {
	...
	tests := []struct {
		...
		{request{operation: operationEnqueue, value: 42}, statusEnqueued, 0},
		{request{operation: operationEnqueue, value: 43}, statusEnqueued, 0},
		{request{operation: operationEnqueue, value: 44}, statusFull, 0},
