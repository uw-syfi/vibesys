...
an exact bounded stack model. MPMC histories use a reservation-aware model that
splits successful pushes into capacity reservation and publication events
within the original call interval.

Both models store published items as a sequence of values with the top at the
end. Push appends. Pop removes the last published item. Duplicate payloads are
distinct slots: a pop must return the current top, even when another stacked
item has the same bytes. In MPMC, full observes reserved plus published
capacity while empty observes only published items.
...
user-interactive QoS. Payload copying, failed try operations, and FFI
transitions are included in elapsed time. A final drain checks item-count
conservation and a commutative fingerprint of pushed and popped payloads.

The benchmark command runs the correctness gate first. Repeated measurements
report the median successful push-plus-pop rate as `total_ops_per_sec`.

## Ownership

Files copied from `examples/evaluators/stack` are trusted evaluator inputs. The
materialized `src/`, build files, and `stack-candidate.so` are candidate-owned.
