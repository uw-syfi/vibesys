...
pub fn run_probe(
    ...
    let mut producers = (0..producer_count)
        .map(|id| stack.create_producer(id))
        .collect::<Result<Vec<_>, _>>()?;
    let mut consumers = (0..consumer_count)
        .map(|id| stack.create_consumer(id))
        .collect::<Result<Vec<_>, _>>()?;
    ...
    for (tag, length) in probe_lengths(max_value_size).into_iter().enumerate() {
        ...
        require_status(
            producer.push(&input),
            STATUS_OK,
            &format!("push length {length}"),
        )?;
        input.fill(0x5a);

        let mut output = vec![SENTINEL_BYTE; max_value_size.max(1)];
        let mut output_length = SENTINEL_LENGTH;
        let status = consumer.pop_raw(output.as_mut_ptr(), output.len() as u64, &mut output_length);
        require_status(status, STATUS_OK, &format!("pop length {length}"))?;
        if output_length != length as u64 {
            return Err(format!(
                "pop length {length} reported output length {output_length}"
            ));
        }
        if output[..length] != expected {
            return Err(format!("pop length {length} returned corrupted bytes"));
        }
        if output[length..].iter().any(|byte| *byte != SENTINEL_BYTE) {
            return Err(format!(
                "pop length {length} wrote beyond the returned value"
            ));
        }
    }
...
fn check_concurrent_undersized_dequeues(
    ...
    let queue = api.create_stack(8, 64, producer_count, consumer_count)?;
    ...
    thread::scope(|scope| {
            ...
            for sequence in 0..CONCURRENT_PROBE_ITEMS {
                ...
                loop {
                    match producer.push(&value) {
                        ...
                        status => {
                            return Err(format!(
                                "concurrent probe push returned ABI status {status}"
                            ));
                        }
                    }
                }
        ...
        let short_worker = scope.spawn(move || {
            ...
            while short_consumed.load(Ordering::Acquire) < CONCURRENT_PROBE_ITEMS {
                ...
                let mut output_length = SENTINEL_LENGTH;
                let status =
                    consumer.pop_raw(output.as_mut_ptr(), output.len() as u64, &mut output_length);
                match status {
                    STATUS_OK => {
                        if output_length != 16 {
                            return Err(format!(
                                "short concurrent pop returned length {output_length}"
                            ));
                        }
                    ...
                    STATUS_INVALID | STATUS_EMPTY => {
                        if output != unchanged || output_length != SENTINEL_LENGTH {
                            return Err("failed concurrent pop modified caller output".to_string());
                        }
                        thread::yield_now();
                    }
                    ...
                    status => {
                        return Err(format!("short concurrent pop returned ABI status {status}"));
                    }
                }
            }
            Ok(())
        ...
        let full_worker = scope.spawn(move || {
            ...
            while full_consumed.load(Ordering::Acquire) < CONCURRENT_PROBE_ITEMS {
                let (status, length) = consumer.pop(&mut output)?;
                match status {
                    STATUS_OK => {
                    ...
                    status => {
                        return Err(format!("full concurrent pop returned ABI status {status}"));
                    }
                }
            }
            Ok(())
    ...
    if observed.iter().any(|value| !value) {
        return Err("concurrent probe lost one or more pushed values".to_string());
    }
    Ok(())
}
...
fn check_empty_output_is_unchanged(
    ...
    let mut output_length = SENTINEL_LENGTH;
    let status = consumer.pop_raw(output.as_mut_ptr(), output.len() as u64, &mut output_length);
    require_status(status, STATUS_EMPTY, "empty pop")?;
    if output != expected || output_length != SENTINEL_LENGTH {
        return Err("empty pop modified caller-owned output".to_string());
    }
    Ok(())
}
...
fn check_invalid_output_retains_value(
    ...
    require_status(
        ...
        STATUS_OK,
        "push before undersized pop",
    )?;

    let mut output = vec![SENTINEL_BYTE; length];
    let unchanged = output.clone();
    let mut output_length = SENTINEL_LENGTH;
    let status = consumer.pop_raw(output.as_mut_ptr(), (length - 1) as u64, &mut output_length);
    require_status(status, STATUS_INVALID, "undersized pop")?;
    if output != unchanged || output_length != SENTINEL_LENGTH {
        return Err("undersized pop modified caller-owned output".to_string());
    }

    output.fill(SENTINEL_BYTE);
    let status = consumer.pop_raw(output.as_mut_ptr(), output.len() as u64, &mut output_length);
    require_status(status, STATUS_OK, "pop after undersized retry")?;
    if output_length != length as u64 || output != expected {
        return Err("undersized pop did not retain the pushed value".to_string());
    }
    Ok(())
}
