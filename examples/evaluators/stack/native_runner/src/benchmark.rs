    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        match producer.push(&payload) {
            ...
            status => {
                ...
                return Err(format!("push returned invalid ABI status {status}"));
            }
        }
        ...
        match status {
            STATUS_OK => {
                if length != value_size {
                    stop.store(true, Ordering::Relaxed);
                    return Err(format!(
                        "pop returned {length} bytes, expected {value_size}"
                    ));
                }
            ...
            status => {
                stop.store(true, Ordering::Relaxed);
                return Err(format!("pop returned invalid ABI status {status}"));
            }
        }
        attempts = attempts.wrapping_add(1);
    }
    ...
    loop {
        ...
        match status {
            STATUS_OK => {
                ...
                if drained > capacity {
                    return Err(format!("drained more than stack capacity {capacity}"));
                }
                let value = validate_payload(&output[..length])?;
