...
func TestAccuracyTimesOutStuckCandidateOperation(t *testing.T) {
	workspace := compileCandidateFixtureWithDefines(t, "VSPQ_TEST_HANG_CAPACITY_ONE")
	...
	err := runAccuracy(accuracyConfig{
		candidateConfig: candidateConfig{
			workspace: workspace,
			candidate: "priority-queue-candidate.so",
...
func compileCandidateFixture(t *testing.T, retainInput bool) string {
	defines := []string{}
	if retainInput {
		defines = append(defines, "VSPQ_TEST_RETAIN_INPUT")
	}
...
func compileCandidateFixtureWithDefines(t *testing.T, defines ...string) string {
	...
	workspace := t.TempDir()
	library := filepath.Join(workspace, "priority-queue-candidate.so")
...
func TestAccuracyRejectsCandidateThatOnlySupportsMaximumLength(t *testing.T) {
	workspace := compileCandidateFixtureWithDefines(t, "VSPQ_TEST_FIXED_LENGTH_ONLY")
	err := runAccuracy(accuracyConfig{
		candidateConfig: candidateConfig{
			workspace: workspace,
			candidate: "priority-queue-candidate.so",
