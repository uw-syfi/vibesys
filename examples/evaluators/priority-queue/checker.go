...
func queueInputFor(req request) queueInput {
	if req.operation == operationEnqueue {
		...
		return queueInput{Kind: "enqueue", Value: &value, Priority: &priority}
	}
...
func runBoundaryHistory(config accuracyConfig) ([]recordedOperation, error) {
	...
	for value := uint64(0); value < config.capacity; value++ {
		if err := appendOperation(request{operation: operationEnqueue, value: value, priority: config.capacity - value}); err != nil {
			_ = session.close()
			return history, err
		}
	}
	if err := appendOperation(request{operation: operationEnqueue, value: config.capacity, priority: 0}); err != nil {
		_ = session.close()
		return history, err
	}
	...
	for index := uint64(0); index < half; index++ {
		if err := appendOperation(request{operation: operationEnqueue, value: config.capacity + 1 + index, priority: index % 3}); err != nil {
			_ = session.close()
			return history, err
		}
	}
	if err := appendOperation(request{operation: operationEnqueue, value: config.capacity*2 + 1, priority: 0}); err != nil {
		_ = session.close()
		return history, err
	}
