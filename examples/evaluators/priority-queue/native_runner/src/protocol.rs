    ...
    while let Some(mut request) = read_request(&mut file, value_size)? {
        match request.operation {
            OPERATION_ENQUEUE => {
                ...
                match producer.enqueue(request.priority, &request.payload) {
                    STATUS_OK => {
...
fn read_request(file: &mut File, max_value_size: usize) -> Result<Option<Request>, String> {
    ...
    let priority = u64::from_le_bytes(header[8..].try_into().expect("priority field"));
    if operation == OPERATION_DEQUEUE && priority != 0 {
        return Err(format!("dequeue priority field is {priority}, want zero"));
    }
