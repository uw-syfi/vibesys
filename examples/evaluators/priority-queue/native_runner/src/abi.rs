...
type EnqueueFn = unsafe extern "C" fn(ProducerHandle, u64, *const u8, u64) -> u32;
...
impl Api {
    pub fn load(path: &Path) -> Result<Self, String> {
        ...
        unsafe {
            let abi_version: AbiVersionFn = library.symbol(b"vspq_abi_version\0")?;
            ...
            Ok(Self {
                ...
                queue_create: library.symbol(b"vspq_queue_create\0")?,
                queue_destroy: library.symbol(b"vspq_queue_destroy\0")?,
                producer_create: library.symbol(b"vspq_producer_create\0")?,
                producer_destroy: library.symbol(b"vspq_producer_destroy\0")?,
                consumer_create: library.symbol(b"vspq_consumer_create\0")?,
                consumer_destroy: library.symbol(b"vspq_consumer_destroy\0")?,
                enqueue: library.symbol(b"vspq_try_enqueue\0")?,
                dequeue: library.symbol(b"vspq_try_dequeue\0")?,
            })
        }
    }
    ...
    pub fn create_queue(
        ...
        if status != STATUS_OK || handle.is_null() {
            return Err(format!("vspq_queue_create returned status {status}"));
        }
        Ok(Queue {
            api: self.clone(),
            handle,
...
impl Queue {
    pub fn create_producer(&self, id: u32) -> Result<Producer, String> {
        ...
        if status != STATUS_OK || handle.is_null() {
            return Err(format!(
                "vspq_producer_create({id}) returned status {status}"
            ));
        }
        Ok(Producer {
            api: self.api.clone(),
    ...
    pub fn create_consumer(&self, id: u32) -> Result<Consumer, String> {
        ...
        if status != STATUS_OK || handle.is_null() {
            return Err(format!(
                "vspq_consumer_create({id}) returned status {status}"
            ));
        }
        Ok(Consumer {
            api: self.api.clone(),
...
struct ReferenceQueue {
    ...
    values: Mutex<Vec<ReferenceEntry>>,
...
unsafe extern "C" fn reference_enqueue(
    ...
    let mut next_sequence = match queue.next_sequence.lock() {
        Ok(sequence) => sequence,
        Err(_) => return STATUS_INTERNAL_ERROR,
    };
    values.push(ReferenceEntry {
        priority,
