...
func TestCorrectnessRequestContainsCopiedPayload(t *testing.T) {
	const valueSize = 64
	request := request{operation: operationEnqueue, value: 99, priority: 7}
	...
	if got := binary.LittleEndian.Uint64(header[8:]); got != request.priority {
		t.Fatalf("priority = %d, want %d", got, request.priority)
	}
...
func TestNativeReferenceWorkerRoundTrip(t *testing.T) {
	...
	tests := []struct {
		...
		{request{operation: operationEnqueue, value: 41, priority: 9}, statusEnqueued, 0},
		{request{operation: operationEnqueue, value: 42, priority: 1}, statusEnqueued, 0},
		{request{operation: operationEnqueue, value: 43, priority: 2}, statusEnqueued, 0},
		{request{operation: operationEnqueue, value: 44, priority: 0}, statusFull, 0},
		{request{operation: operationDequeue}, statusValue, 42},
		{request{operation: operationDequeue}, statusValue, 43},
