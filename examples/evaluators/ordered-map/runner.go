...
func nativeRunnerPath() (string, error) {
	runnerOnce.Do(func() {
		if configured := os.Getenv("VIBESYS_ORDERED_MAP_NATIVE_RUNNER"); configured != "" {
		...
		target := filepath.Join(
			os.TempDir(),
			fmt.Sprintf("vibesys-ordered-map-native-%x", digest[:8]),
		)
		...
		runnerPath = filepath.Join(target, "release", "vibesys-ordered-map-native-runner")
		runnerErr = validateRunnerExecutable(runnerPath)
	})
