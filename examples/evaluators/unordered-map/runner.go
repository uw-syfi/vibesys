...
func nativeRunnerPath() (string, error) {
	runnerOnce.Do(func() {
		if configured := os.Getenv("VIBESYS_UNORDERED_MAP_NATIVE_RUNNER"); configured != "" {
		...
		target := filepath.Join(
			os.TempDir(),
			fmt.Sprintf("vibesys-unordered-map-native-%x", digest[:8]),
		)
		...
		runnerPath = filepath.Join(target, "release", "vibesys-unordered-map-native-runner")
		runnerErr = validateRunnerExecutable(runnerPath)
	})
