...
func startCandidate(config candidateConfig) (*candidateSession, error) {
	...
	if config.clientCount <= 0 {
		return nil, errors.New("candidate session requires a positive client count")
	}
	...
	for lane := 0; lane < config.laneCount; lane++ {
		trusted, runner, err := createSocketPair(fmt.Sprintf("map-lane-%d", lane))
		if err != nil {
...
func (s *candidateSession) invoke(laneIndex int, req request) (response, error) {
	...
	if err := writeRequest(lane.conn, req, s.keySize, s.valueSize); err != nil {
