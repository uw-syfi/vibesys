...
Exit protocol (mirrors kv-store/accuracy_checker/checker.py):
  ...
  * exit 1  — at least one gate FAILED, or a required reference-backed gate
              could not run. Correctness cannot be established without the
              pristine engine.
  * exit 2  — (only with --strict) a gate could not run (SETUP-ERROR) and no gate
              FAILED. By default a SETUP-ERROR from an optional environmental gate
              (e.g. no nightly toolchain for the sanitizer, or a crash could not be
...
def main() -> int:
    ...
    for name in selected:
        ...
        if rc == _PASS:
            continue
        if rc == _SETUP:
            if name in _REQUIRED_GATES:
                required_setup.append(name)
            else:
                any_setup = True
        else:
            any_fail = True
