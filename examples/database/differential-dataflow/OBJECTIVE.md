...
- **Evaluator files are immutable.** Do not edit `_ref_engine/`, `reference/`,
  `accuracy_checker/`, `benchmark/`, `profiler/`, `vibesys.input.toml`,
  `objectives.toml`, or this objective. A round may change only the `engine/`
  paths listed under "What you may edit". The behavioral checker enforces output
  and runtime properties; the independent judge must also inspect the Git diff
  for this path rule and the architectural walls above.
