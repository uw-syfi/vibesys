...
engine offline, times it on the fixed metric workload exactly as `benchmark.py`
will time every candidate, and writes `baseline.json`. Because every later round
starts from this same source and only micro-optimizes it,
...
def main():
    print("capturing round-0 (vanilla _ref_engine) CPU baseline")
    print(f"  _ref_engine : {_REF_ENGINE}")
    ...
    data = {
        ...
        "engine": "differential-dataflow (vanilla _ref_engine, round 0)",
        "host": platform.node(),
