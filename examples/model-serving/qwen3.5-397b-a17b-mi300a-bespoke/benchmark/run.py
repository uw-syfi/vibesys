...
- ``--vs-output``: the VibeSys evaluator result protocol v2 record stream (see
  ...
  and ``p95_ttft_turn2plus_ms`` (min), are read from one run.
...
# must be a required metric, so both stay required here. ``mean_tpot_ms`` and
# ``turn1_ttft_ms`` are the only two that can come out NaN (their sample sets
# can be empty), so they are the only optional ones; the rest are counts,
# fractions, or echoed settings that are always finite.
#
# ``direction`` is advisory metadata about each metric, not the objective
# selection: ``objectives.toml`` owns which of these axes are ranked.
METRIC_SPECS: dict[str, MetricSpec] = {
    "p95_ttft_turn2plus_ms": MetricSpec(unit="ms", direction="min"),
    "mean_tpot_ms": MetricSpec(unit="ms", direction="min", required=False),
    # Output (completion) tokens per second summed over every concurrent
    # session, divided by the whole run's wall clock. See aggregate_metrics.
    "total_token_throughput": MetricSpec(unit="tok/s", direction="max"),
    "turn1_ttft_ms": MetricSpec(unit="ms", direction="min", required=False),
    ...
    "pacing_scheduled": MetricSpec(),
    "ref_ttft_ms": MetricSpec(unit="ms"),
    "ref_tpot_ms": MetricSpec(unit="ms"),
    "concurrency": MetricSpec(unit="sessions"),
    "schedule_bound_fraction": MetricSpec(),
    "offered_turn_rate_per_s": MetricSpec(unit="turns/s"),
...
def aggregate_metrics(
    ...
    *,
    ...
    - It counts completion tokens only, not prompt tokens, which is what
      ...
      tokens and calls this quantity ``output_throughput``.)
    - The denominator is the whole run's wall clock, including the session
      start stagger and think time, so this is a run-level rate rather than a
      saturated-server capacity. Under ``--pacing scheduled`` the numerator is
      a constant (every turn must fill its fixed budget), so the metric is
      ``constant / makespan`` and saturates once the server is fast enough for
      every send to be schedule-bound. It then measures "does the server keep
      up with the offered load" and stops separating servers that do; the
      latency percentiles remain the fine-grained axis.
    """
...
def summary_line(pacing: str, values: dict[str, float]) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path the human-readable metrics JSON is written to (only on a "
            "fully clean run). Defaults to '<--vs-output>.metrics.json'. At "
            "least one of --output-json and --vs-output is required."
        ),
    )
    parser.add_argument(
