...
a fake HTTP server, ``aggregate_metrics``'s reduction, and the evaluator result
...
class VsProtocolReportTests(unittest.TestCase):
    ...
    def test_hello_is_flushed_before_the_outcome_is_known(self) -> None:
        # PROTOCOL.md: a producer killed after hello leaves NO_OUTCOME ("started
        # and died"), which the reader distinguishes from MISSING_HELLO.
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "stream.jsonl"
...
class DeclaredMetricsTests(unittest.TestCase):
    """The declared schema against objectives.toml and aggregate_metrics."""
    ...
    def test_objectives_are_declared_and_required(self) -> None:
        # PROTOCOL.md: every objective must appear in hello.metrics and must be
        # required, or the framework rejects the round.
        names = [entry["name"] for entry in self._objectives()]
        self.assertEqual(names, ["total_token_throughput", "p95_ttft_turn2plus_ms"])
        for name in names:
    ...
    def test_only_the_nan_capable_metrics_are_optional(self) -> None:
        optional = {name for name, spec in run.METRIC_SPECS.items() if not spec.required}
        self.assertEqual(optional, {"mean_tpot_ms", "turn1_ttft_ms"})

    def test_aggregate_metrics_reports_exactly_the_declared_metrics(self) -> None:
        results = [
            run.TurnResult(0, 1, True, ttft_s=0.01, completion_tokens=10, latency_s=0.5),
            run.TurnResult(0, 2, True, ttft_s=0.02, completion_tokens=20, latency_s=0.6),
        ]
        values = run.aggregate_metrics(
            results, wall_s=2.0, pacing="scheduled", ref_ttft_ms=700.0, ref_tpot_ms=110.0
        )
        self.assertEqual(set(values), set(run.METRIC_SPECS))

    def test_total_token_throughput_is_aggregate_tokens_over_wall_clock(self) -> None:
        # Tokens from every session, over the whole run's wall clock: two
        # sessions' turns must both count against the one shared denominator.
        results = [
            run.TurnResult(0, 1, True, ttft_s=0.01, completion_tokens=100, latency_s=0.5),
            run.TurnResult(1, 1, True, ttft_s=0.01, completion_tokens=140, latency_s=0.5),
            run.TurnResult(1, 2, True, ttft_s=0.01, completion_tokens=60, latency_s=0.5),
        ]
        values = run.aggregate_metrics(
            results, wall_s=4.0, pacing="scheduled", ref_ttft_ms=700.0, ref_tpot_ms=110.0
        )
        self.assertAlmostEqual(values["total_token_throughput"], 300 / 4.0)
...
class MainAsyncProtocolTests(unittest.TestCase):
    """main_async's record stream, with the measurement itself stubbed out."""

    def _args(self, tmp: str, **overrides) -> object:
        import argparse
    ...
    def _values(self, **overrides) -> dict[str, float]:
        values = {name: 1.0 for name in run.METRICS}
        values["total_token_throughput"] = 275.5
        values["p95_ttft_turn2plus_ms"] = 812.0
    ...
    def test_success_writes_hello_then_result_with_both_objectives(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            args = self._args(tmp)
            values = self._values()
            with mock.patch.object(run, "measure", mock.AsyncMock(return_value=(values, []))):
            ...
            self.assertEqual(records[0]["metrics"]["total_token_throughput"]["direction"], "max")
            self.assertEqual(records[0]["metrics"]["p95_ttft_turn2plus_ms"]["direction"], "min")
            self.assertEqual(records[1]["label"], "")
            self.assertEqual(records[1]["values"]["total_token_throughput"], 275.5)
            self.assertEqual(records[1]["values"]["p95_ttft_turn2plus_ms"], 812.0)

    def test_success_also_writes_the_human_readable_json_beside_the_stream(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            args = self._args(tmp)
            values = self._values()
            with mock.patch.object(run, "measure", mock.AsyncMock(return_value=(values, []))):
            ...
            self.assertEqual(json.loads(metrics_path.read_text()), values)
            self.assertTrue(run.turns_output_path(metrics_path).exists())

    def test_nan_optional_metric_is_left_out_of_the_row(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            args = self._args(tmp)
            values = self._values(mean_tpot_ms=float("nan"))
            with mock.patch.object(run, "measure", mock.AsyncMock(return_value=(values, []))):
                self.assertEqual(asyncio.run(run.main_async(args)), 0)
            row = self._records(Path(args.vs_output))[1]["values"]
            self.assertNotIn("mean_tpot_ms", row)
            self.assertIn("p95_ttft_turn2plus_ms", row)

    def test_failure_writes_hello_then_error_and_exits_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            args = self._args(tmp)
            failure = mock.AsyncMock(side_effect=RuntimeError("3 failed turn(s) of 214 planned"))
            with mock.patch.object(run, "measure", failure):
                code = asyncio.run(run.main_async(args))
            self.assertEqual(code, 1)
            records = self._records(Path(args.vs_output))
            self.assertEqual([record["kind"] for record in records], ["hello", "error"])
            self.assertIn("3 failed turn(s)", records[1]["message"])
    ...
    def test_without_vs_output_only_the_human_readable_json_is_written(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                run, "measure", mock.AsyncMock(return_value=(self._values(), []))
            ):
                self.assertEqual(asyncio.run(run.main_async(args)), 0)
