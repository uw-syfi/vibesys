...
func TestAccuracyCaseCountRejectsNamespaceOverlapBeforeChecks(t *testing.T) {
	for _, cases := range []int{1, maxAccuracyCases} {
		if err := validateAccuracyCaseCount(cases); err != nil {
			t.Fatalf("case count %d was rejected: %v", cases, err)
		}
	}
	for _, cases := range []int{0, maxAccuracyCases + 1} {
		if err := validateAccuracyCaseCount(cases); err == nil {
			t.Fatalf("case count %d was accepted", cases)
		}
	}

	invoked := false
	runtime := runtimeFunc(func(context.Context, api.Invocation) api.ProtocolResult {
		invoked = true
	...
	err := (&Application{}).Check(
		context.Background(),
		runtime,
		api.AccuracyContext{Cases: maxAccuracyCases + 1},
		nil,
	)
	if err == nil || !strings.Contains(err.Error(), "overlap reserved date namespaces") {
		t.Fatalf("case-count error=%v", err)
	}
	if invoked {
		t.Fatal("Hotel checks started before the case count was validated")
	}
}
