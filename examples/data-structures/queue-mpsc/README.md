...
mutex and `VecDeque`. Build and validate it from this directory:
    ...
    make
    go -C ../../libs/queue-input-core/src/queue_input_core/trusted_harness run . check --workspace "$PWD" --scenario mpsc
    go -C ../../libs/queue-input-core/src/queue_input_core/trusted_harness run . benchmark --workspace "$PWD" --scenario mpsc --duration 1s --warmup 0s
