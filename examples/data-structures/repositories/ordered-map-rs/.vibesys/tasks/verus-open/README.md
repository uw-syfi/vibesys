# Open Verus Ordered Map Task

This task explores proof-carrying generation of a concurrent ordered map in
Rust and Verus. It uses the candidate library at `verus-ordered-map/` directly, with no
...
3. The task-owned `accuracy/` crate checks the fixed public interface, sequential
   put/get/remove, min/max/predecessor/successor/range snapshots, concurrent
   disjoint inserts, and concurrent-remove conservation.

The separate task-owned `benchmark/` crate runs a mixed 4-client workload
including get, put, remove, successor, and range over a 256-key universe and
reports `total_ops_per_sec`. Neither crate is part of the verified candidate.
...
Run the task from the `ordered-map-rs` repository root:
...
uv run vibesys \
  --outer-loop agent \
  --project examples/data-structures/repositories/ordered-map-rs \
  --task verus-open \
  --runs-dir /absolute/path/to/vibesys-runs --local \
  --backend cpu --profiler none \
  --max-rounds 4
