...
fn sequential_order_contract() -> Result<(), String> {
    let map = OrderedMap::new();
    map.put(2, 20);
    map.put(4, 40);
    map.put(6, 60);
    if map.min() != Some((2, 20)) || map.max() != Some((6, 60)) {
        return Err("min and max must be the endpoints of the sorted map".to_owned());
    }
    if map.predecessor(4) != Some((2, 20)) || map.successor(4) != Some((6, 60)) {
        return Err("predecessor and successor must be strict neighbors".to_owned());
    }
    if map.predecessor(2).is_some() || map.successor(6).is_some() {
        return Err("missing neighbors must return None".to_owned());
    }
    let (items, remaining) = map.range(2, 6, 8);
    if items != vec![(2, 20), (4, 40)] || remaining {
        return Err("range must return a sorted snapshot of [start, end)".to_owned());
    }
    let (truncated, more) = map.range(0, 100, 1);
    if truncated != vec![(2, 20)] || !more {
        return Err("range must set remaining when truncated".to_owned());
    }
    Ok(())
}
...
fn check() -> Result<(), String> {
    sequential_contract()?;
    sequential_order_contract()?;
    concurrent_put_get_contract()?;
    concurrent_remove_contract()?;
    println!("PASS - pure-Rust exact-linearizable ordered map checks");
    Ok(())
}
