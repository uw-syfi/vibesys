...
#[no_mangle]
pub extern "C" fn vsst_abi_version() -> u32 {
    ABI_VERSION
}

#[no_mangle]
pub unsafe extern "C" fn vsst_stack_create(
    capacity: u64,
    max_value_size: u64,
    producer_count: u32,
    consumer_count: u32,
    stack_out: *mut *mut Stack,
) -> u32 {
    if stack_out.is_null() {
        return STATUS_INVALID;
    }
    let stack = match Stack::new(capacity, max_value_size, producer_count, consumer_count) {
        Ok(stack) => stack,
        Err(status) => return status,
    };

    unsafe { stack_out.write(Box::into_raw(Box::new(stack))) };
    STATUS_OK
}

#[no_mangle]
pub unsafe extern "C" fn vsst_stack_destroy(stack: *mut Stack) {
    if !stack.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn vsst_producer_create(
    stack: *mut Stack,
    producer_id: u32,
    producer_out: *mut *mut Producer,
) -> u32 {
    let Some(stack) = (unsafe { stack.as_ref() }) else {
        return STATUS_INVALID;
    };
    if producer_out.is_null() {
        return STATUS_INVALID;
    }
    let producer = match stack.producer(producer_id) {
...
#[no_mangle]
pub unsafe extern "C" fn vsst_producer_destroy(producer: *mut Producer) {
    if !producer.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn vsst_consumer_create(
    stack: *mut Stack,
    consumer_id: u32,
    ...
    let consumer = match stack.consumer(consumer_id) {
...
#[no_mangle]
pub unsafe extern "C" fn vsst_consumer_destroy(consumer: *mut Consumer) {
    if !consumer.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn vsst_try_push(
...
#[no_mangle]
pub unsafe extern "C" fn vsst_try_pop(
