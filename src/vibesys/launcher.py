...
def launch(argv: list[str]) -> int:
    if any(argument in {"-h", "--help"} for argument in argv):
        return subprocess.call([sys.executable, "-m", "vibesys.cli", *argv, "--headless"])

    runtime = os.environ.get("VIBESYS_TUI_RUNTIME") or shutil.which("bun")
    entrypoint = PROJECT_ROOT / "clients" / "tui" / "dist" / "index.js"
    if runtime is None:
        print("vibesys-launch: Bun is required by the OpenTUI client.", file=sys.stderr)
        return 1
    if not entrypoint.is_file():
        print("vibesys-launch: TUI build is missing; run `./vs ...`.", file=sys.stderr)
        return 1
...
def _report_backend_failure(backend: subprocess.Popen[bytes], log_path: Path) -> None:
    ...
    try:
        ...
        tail = []
    print(
        f"vibesys-launch: backend exited with status {backend.returncode or 1}",
        file=sys.stderr,
    )
    if tail:
