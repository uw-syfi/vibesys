"""Pure policy controller for profile-guided hypothesis selection."""
...
def persist_agent_run_state(
    ...
    *,
    ...
    """Persist authoritative agent state without staging candidate edits."""
    store.save(state)
    ctx.state.commit(label, store.namespace)


def persist_active_hypothesis(
    ctx: LoopContext,
    store: AgentRunStateStore,
    state: AgentRunState,
    ...
    *,
    ...
    """Replace and persist the active hypothesis inside its aggregate."""
