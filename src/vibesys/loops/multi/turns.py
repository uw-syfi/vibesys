...
from vibesys.events import CoreEventType, EventStatus, JudgeResultData
...
class MultiAgentTurns:
    ...
    def _profiler(self) -> ProfilerDefinition | None:
        ...
        return definition
    async def _read_only(self, agent, *, message: str, prompt: str, response_cls, fallback_factory, label: str, allowed: tuple[str, ...] = (), mcp_servers=None):
        revision = await self.workspace.snapshot(f"{label}-input")
        try:
            ...
            changed = await self.workspace.pending_changes()
            unauthorized = [path for path in changed if not any(path == item or path.startswith(f"{item.rstrip('/')}/") for item in allowed)]
            if unauthorized:
                await self.workspace.restore(revision, clean=True, preserve_paths=allowed)
                self.ctx.log(f"[role-isolation] reverted {len(unauthorized)} workspace change(s): {', '.join(unauthorized[:8])}")

    async def pre_round_decision(self, round_number: int, carry, *, has_history: bool) -> PreRoundDecision:
        """Ask the orchestrator whether a specialist profile is useful."""
        view = self.ctx.environment.view
        prompt = render_template(
            "orchestrator_pre_round_prompt.j2", template_dir=self.template_dir,
            objective=self.objective, objective_location=view.paths.objective,
            regression_info=carry.regression_info, exhaustion_info=carry.exhaustion_info,
            progress_location=self.progress_location,
            profiler_kind=self.ctx.environment.profiler_kind.value,
            profile_execution=view.profile_execution, has_history=has_history,
        )
        decision = await self._read_only(
            self.designer, message="Decide whether a profiling pass is needed before planning this round. Return only the JSON object.",
            prompt=prompt, response_cls=PreRoundDecision,
            fallback_factory=lambda: PreRoundDecision(need_profile=False, profile_focus="", reasoning="fallback: default to skip"),
    ...
    async def profile(self, round_number: int, focus: str) -> ProfilerSummary | None:
        ...
        view = self.ctx.environment.view
        prompt = render_template(
            definition.prompt_template, template_dir=self.template_dir,
            profile_focus=focus, benchmark_command=view.paths.benchmark_command,
            modality=self.modality,
            domain_profiler=render_domain_section(self.domain, DomainRole.PROFILER, **self._domain_context()),
            runtime_notes=view.prompt_notes, profile_execution=view.profile_execution,
            objective=self.objective, profiler_support_name=definition.support_name,
            profiler_mcp_name=definition.mcp_name,
        )
        artifact = issue_board.display_path(issue_board.profiler_artifact_root(self.progress_path, round_number), self.workspace.path).rstrip("/")
        prompt += (
            f"\n\n## Recent campaign context\nRead `{self.progress_location}` and current retained evidence before new profiling."
            f"\n\n## Read-only evidence boundary\nDo not edit candidate source, configuration, tests, or entrypoints."
            f" Write bounded durable profile evidence only below `{artifact}/`; keep large traces under `/tmp`."
        )
        spec = profiler_mcp_spec(self.ctx.environment.profiler_kind)
        try:
            summary = await self._read_only(
                self.profiler, message="Profile the server and return exactly one JSON object matching the schema above.",
                prompt=prompt, response_cls=ProfilerSummary,
                fallback_factory=lambda: ProfilerSummary(analysis="Profiler produced no structured response.", bottlenecks="n/a", suggestions="Re-run profiling on the next round."),
                label=f"round-{round_number}-profiler", allowed=(artifact,),
                mcp_servers=[spec] if spec is not None else None,
            )
        except Exception as error:
            output_sink().framework_warning("profiler failed", detail=str(error), source=FrameworkSource.LOOP, round_label=f"round-{round_number}")
            return None
    ...
    def _implementer_prompt(self, request: AttemptRequest, state: AttemptState, skills: list) -> str:
        plan = request.plan
        ...
        prior = tuple(issue_board.display_path(path, self.workspace.path) for path in issue_board.implementer_artifact_paths(self.progress_path, request.round_number))
        location = lambda path: issue_board.display_path(path, self.workspace.path)
        return render_template(
            "implementer_continuation_prompt.j2" if hypothesis.next_step else "implementer_prompt.j2",
            template_dir=self.template_dir, reference_path=self.ctx.environment.reference_path,
            modality=self.modality, interface=self.options.interface,
            domain_implementer=render_domain_section(self.domain, DomainRole.IMPLEMENTER, **self._domain_context()),
            task=plan.task, pass_criteria=plan.pass_criteria, objective=self.objective,
            objective_location=view.paths.objective, plan_artifact_location=location(artifact),
            hypothesis_id=plan.hypothesis_id, hypothesis=plan.hypothesis,
            activation_evidence=plan.activation_evidence, falsification_criteria=plan.falsification_criteria,
            expected_effect=plan.expected_effect, minimum_acceptance_criteria=plan.minimum_acceptance_criteria,
            invariants=plan.invariants, progress_location=self.progress_location,
            pareto_archive_location=self.pareto_location,
            validation_location=location(issue_board.validation_artifact_root(self.progress_path)),
            validation_recipe_contract_location=location(issue_board.validation_recipe_schema_path(self.progress_path)),
            retry=state.retry, feedback=state.feedback, continuation_step=hypothesis.next_step,
            framework_revert_applied=hypothesis.revert_applied, framework_revert_round=hypothesis.parent_round,
            ...
            recommended_skills=skills, prior_attempt_artifact_locations=prior,
    ...
    async def implement(self, request: AttemptRequest, state: AttemptState) -> tuple[ImplementerResponse, bool]:
        """Run one paid implementation attempt with a durable marker already written."""
        import subprocess
        plan = request.plan
        ...
        def fallback() -> ImplementerResponse:
            ...
            return ImplementerResponse(
                summary="Implementer produced no structured response.", expected_behavior="unknown",
                hypothesis_outcome="inconclusive", evidence="The implementer output could not be parsed.",
        ...
        try:
            response = await self.worker.turn_structured(
                "Execute the required continuation step and return only the JSON object." if request.active_hypothesis.next_step else "Work persistently on the active hypothesis and return only the JSON object.",
                system_prompt=prompt, response_cls=ImplementerResponse, fallback_factory=fallback,
                label=f"round-{request.round_number}-retry-{state.retry}-implementer", reuse_session=True,
                session_key=AgentSessionKey(SessionScope.HYPOTHESIS, plan.hypothesis_id),
            )
        except subprocess.TimeoutExpired as error:
            synthesized = True
            response = ImplementerResponse(
                summary="Implementer invocation timed out.", expected_behavior="unknown",
            ...
            self.ctx.log(f"[implementer] attempt {state.retry} timed out after {error.timeout:g} seconds")
        response.skill_context_updates, _ = self._skills(response.skill_context_updates)
        if response.skill_context_updates:
            plan.recommended_skills, _ = self._skills([*plan.recommended_skills, *response.skill_context_updates])
            issue_board.write_plan_artifact(self.progress_path, request.round_number, plan)
        issue_board.write_implementer_artifact(self.progress_path, request.round_number, state.retry, response)
        issue_board.append_implementer(self.progress_path, request.round_number, state.retry, response)
        await self.workspace.snapshot(f"round-{request.round_number}-retry-{state.retry}-implementer")
        return response, synthesized

    async def review(self, request: AttemptRequest, state: AttemptState, conflict: str | None) -> JudgeResponse:
        """Ask the independent judge to audit the parsed implementation."""
        implementation = state.implementation
        if implementation is None:
            raise ValueError("judge requires a parsed implementer response")
        plan = request.plan
        hypothesis = request.active_hypothesis
        view = self.ctx.environment.view
        location = lambda path: issue_board.display_path(path, self.workspace.path)
        plan_artifact = issue_board.write_plan_artifact(self.progress_path, request.round_number, plan)
        evidence = issue_board.write_implementer_artifact(self.progress_path, request.round_number, state.retry, implementation)
        domain_context = self._domain_context()
        domain_context["accuracy_command"] = None
        domain_context["benchmark_command"] = None
        prompt = render_template(
            "judge_prompt.j2", template_dir=self.template_dir,
            accuracy_command=view.paths.accuracy_command, benchmark_command=view.paths.benchmark_command,
            pass_criteria=plan.pass_criteria, modality=self.modality, interface=self.options.interface,
            domain_judge=render_domain_section(self.domain, DomainRole.JUDGE, **domain_context),
            retry=state.retry, runtime_notes=view.prompt_notes, profile_execution=view.profile_execution,
            objective=self.objective, objective_location=view.paths.objective,
            plan_artifact_location=location(plan_artifact), implementer_artifact_location=location(evidence),
            hypothesis_id=plan.hypothesis_id, hypothesis=plan.hypothesis,
            activation_evidence=plan.activation_evidence, falsification_criteria=plan.falsification_criteria,
            expected_effect=plan.expected_effect, minimum_acceptance_criteria=plan.minimum_acceptance_criteria,
            invariants=plan.invariants, implementer_outcome=implementation.hypothesis_outcome.value,
            implementer_evidence=implementation.evidence, implementer_perf_metric=implementation.perf_metric,
            implementer_perf_unit=implementation.perf_unit, implementer_metrics=implementation.metrics,
            ...
            gate_approved_evaluation_artifact=hypothesis.gate_approved_evaluation_artifact,
            progress_location=self.progress_location, pareto_archive_location=self.pareto_location,
            validation_location=location(issue_board.validation_artifact_root(self.progress_path)),
            validation_recipe_contract_location=location(issue_board.validation_recipe_schema_path(self.progress_path)),
            framework_revert_applied=hypothesis.revert_applied, framework_revert_round=hypothesis.parent_round,
            framework_revert_commit=hypothesis.revert_commit,
        ...
        response = await self._read_only(
            self.judge, message="Review the implementation per the criteria above. Return only the JSON verdict.",
            prompt=prompt, response_cls=JudgeResponse,
            fallback_factory=lambda: JudgeResponse(analysis="Judge produced no structured response.", feedback="No structured response received.", verdict=Verdict.FAIL),
        ...
        if response.verdict is Verdict.PASS and conflict:
            response = response.model_copy(update={"analysis": f"{response.analysis}\n\nFramework Pareto guard: {conflict}", "feedback": conflict, "verdict": Verdict.FAIL})
        self.ctx.events.emit(
            CoreEventType.JUDGE_RESULT,
            status=EventStatus.COMPLETED if response.verdict is Verdict.PASS else EventStatus.FAILED,
            round_label=f"round-{request.round_number}-retry-{state.retry}", agent_kind="judge",
            data=JudgeResultData(verdict=response.verdict.value, feedback=response.feedback, attempt=state.retry),
        )
