"""Durable round control for the profile guided single strategy over public host capabilities."""
...
class ProfileSingleSession:
    """Own the profile guided single strategy's state, roles, attempts, and checkpoints."""
    ...
    @classmethod
    async def open(
        cls, ctx: RunContext, options: AgentOrchestrationOptions
    ...
    async def _initialize(self) -> None:
        ...
        if previous != state:
            await self._save_state(state, label="profile_single: initialize policy state")
    ...
    async def select_hypothesis(self) -> ProfileSingleRound:
        ...
        if hypothesis is None:
            ...
            await self._save_state(
                state, label=f"profile_single: start hypothesis {plan.hypothesis_id}"
            )
    ...
    async def _apply_rollback(self, selection: RoundSelection) -> None:
        ...
        await self._save_state(
            state, label=f"profile_single: set hypothesis {hypothesis.hypothesis_id} parent"
        )
        self.state = state
    ...
    def remaining_attempts(self, selected: ProfileSingleRound) -> range:
        ...
        if first > limit:
            raise ProfileSingleSessionError.exhausted_attempts(self.round_number, first, limit)
        if first > 1:
    ...
    async def official_gates(self, selected: ProfileSingleRound) -> bool:
        """Evaluate a candidate only after the combined role passes itself."""
        return await self._official_gates(selected)

    async def _checkpoint_active(self, selected: ProfileSingleRound) -> None:
    ...
    def _record_official_decision(
        self, selected: ProfileSingleRound, *, run: bool, reason: str
    ) -> None:
    ...
    async def _official_gates(self, selected: ProfileSingleRound) -> bool:
        ...
        if reason is None:
            raise ProfileSingleSessionError.missing_gate_reason()
