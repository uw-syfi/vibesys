...
Also holds the errors shared by the four hypothesis-driven agent strategies
(multi, single, profile_multi, profile_single). Each strategy previously
defined its own byte-for-byte (or near-identical) copy of these classes
purely because its ``turns.py``/``session.py``/``orchestration.py`` couldn't
import a sibling strategy's module. They all import this one shared module
instead; strategies still never import each other.
"""
