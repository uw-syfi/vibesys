...
from vibe_sys.config import Config, _load_config
from vibe_sys.constants import (
    KNOWN_COMPUTE_BACKENDS,
    PROJECT_ROOT,
    ComputeBackend,
)
from vibe_sys.errors import ConfigurationDiagnostic, ConfigurationError
from vibe_sys.input_manifest import InputBundle, load_input_bundle
from vibe_sys.profilers import CLI_PROFILER_CHOICES, ProfilerKind, coerce_profiler_kind
from vibe_sys.sandbox.run_environment import (
    RunEnvironmentSpec,
    make_run_environment_spec,
)
from vibe_sys.skills import DEFAULT_SKILL_ROOTS, resolve_skill_source_dirs
...
def _fail(msg: str) -> None:
    raise ConfigurationError(
        ConfigurationDiagnostic(
            ...
            message=msg,
            usage=(
                f"Usage: vibe-sys --outer-loop {{{'|'.join(_OUTER_LOOPS)}}} [loop-specific args...]"
            ),
        )
    )
...
def _build_agent_parser() -> argparse.ArgumentParser:
    parser = _make_parser(
        prog="vibe-sys --outer-loop agent",
...
def _run_agent(args: argparse.Namespace) -> None:
    ...
    from vibe_sys.loops.agent.loop import run_agent_loop
...
def _parse_cli_objective(spec: str):
    ...
    from vibe_sys.loops.evolve.population import Objective
...
def _build_evolve_parser() -> argparse.ArgumentParser:
    parser = _make_parser(
        prog="vibe-sys --outer-loop evolve",
...
def _run_evolve(args: argparse.Namespace) -> None:
    config, skills, backend = load_config_and_skills(args)
    from vibe_sys.loops.evolve.loop import run_evolve_loop
...
def _build_openevolve_parser() -> argparse.ArgumentParser:
    parser = _make_parser(
        prog="vibe-sys --outer-loop openevolve",
        description=(
...
def _build_plain_parser() -> argparse.ArgumentParser:
    parser = _make_parser(
        prog="vibe-sys --outer-loop plain",
        description=(
...
def _dispatch(argv: list[str]) -> None:
    ...
    from vibe_sys.server.events import EventStatus, EventType, RunStartedData
    from vibe_sys.server.registry import active_supervisor
...
def _render_configuration_error(error: ConfigurationError) -> NoReturn:
    ...
    print(f"vibe-sys: {diagnostic.message}", file=sys.stderr)
    if diagnostic.usage:
...
def main() -> None:
    ...
    if interactive:
        from vibe_sys.launcher import launch
