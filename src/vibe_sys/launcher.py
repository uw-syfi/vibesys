"""Launch one headless VibeSys backend and one terminal client."""
...
from vibe_sys.constants import PROJECT_ROOT
...
def launch(argv: list[str]) -> int:
    if any(argument in {"-h", "--help"} for argument in argv):
        return subprocess.call([sys.executable, "-m", "vibe_sys.cli", *argv, "--headless"])

    runtime = os.environ.get("VIBESYS_TUI_RUNTIME") or shutil.which("bun")
    entrypoint = PROJECT_ROOT / "clients" / "tui" / "dist" / "index.js"
    if runtime is None:
        print("vibe-sys-launch: Bun is required by the OpenTUI client.", file=sys.stderr)
        return 1
    if not entrypoint.is_file():
        print("vibe-sys-launch: TUI build is missing; run `./vs ...`.", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="vibesys-session-") as session_dir:
        ...
        with backend_log_path.open("w", encoding="utf-8") as backend_log:
            ...
            try:
                ...
                env = os.environ.copy()
                env["VIBESYS_CONTROL_SOCKET"] = str(socket_path)
...
def _monitor(
    ...
    while True:
        ...
        if backend_code is not None:
            # Once attached, the frontend owns terminal presentation, including
            # terminal run failures. Keep it open for inspection until the user
            # exits instead of corrupting the display with captured stderr.
            return frontend.wait() or backend_code
        time.sleep(0.05)
...
def _report_backend_failure(backend: subprocess.Popen[bytes], log_path: Path) -> None:
    ...
    try:
        ...
        tail = []
    print(
        f"vibe-sys-launch: backend exited with status {backend.returncode or 1}",
        file=sys.stderr,
    )
    if tail:
