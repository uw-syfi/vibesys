...
from vibe_sys.profilers import ProfilerKind, profiler_definition, require_profiler_kind
from vibe_sys.schemas import ProfilerSummary


def mcp_spec(profiler_kind: ProfilerKind):
    """Build an ``MCPServerSpec`` that spawns the analysis MCP server.

    Returns ``None`` when ``vibe_sys._agent_cli`` is not importable in the
