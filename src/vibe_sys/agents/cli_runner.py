...
Wraps the local ``vibe_sys._agent_cli`` compatibility layer, which is backed by
...
5. Reuses :func:`vibe_sys.agent_runner._parse_typed_response_text`
...
from vibe_sys._agent_cli.base import CodingAgent, MCPServerSpec
...
from vibe_sys.agents.progress import AgentProgress
...
# vibe-sys-skills install.sh conventions. Each CLI tool auto-loads
...
def _discover_skill_dirs(root: Path) -> list[Path]:
    ...
    the tier-organized layout from vibe-sys-skills
...
class CliAgentRunner:
    """:class:`AgentRunner` backed by ``vibe_sys._agent_cli`` CLI agents."""
    ...
    def invoke(
        ...
        *,
        ...
        if agent is not None:
            ...
            from vibe_sys.agents.modal_executor import ModalCommandExecutor
