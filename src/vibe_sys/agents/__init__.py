...
runner classes are imported, so the public API of this package is::
    ...
    from vibe_sys.agents import AgentRunner, RoundProgress, build_agent_runner
"""
...
from vibe_sys.constants import DEFAULT_AGENT_BACKEND
...
# circular import: ``vibe_sys.agent_runner`` imports
# ``vibe_sys.agents.callbacks`` (which triggers this ``__init__``), and both
# concrete runners import back from ``vibe_sys.agent_runner``. Importing
# them eagerly here turned the cycle into an ImportError on the first entry
# point that hits ``agent_runner`` first (e.g. the ``vibe-sys`` script).
__all__ = [
    "AgentProgress",
    "AgentRunner",
    "CandidateProgress",
...
def build_agent_runner(
    ...
    *,
    ...
    Args:
        config: Parsed :class:`~vibe_sys.config.Config`; the ``[agent]``
        ...
        agent_backend: CLI override; if set, takes precedence over
            ...
            :data:`vibe_sys.constants.DEFAULT_AGENT_BACKEND` (``"cli"``).
        ...
        skill_source_dirs: Absolute source paths of skill directories — the
            ...
            on each invoke.
        model: Result of :func:`vibe_sys.llm_client._build_model` — only
