...
- :mod:`vibe_sys.agents.deepagents_runner` wraps the existing
  ``deepagents`` + ``langchain`` stack used by every loop today.
- :mod:`vibe_sys.agents.cli_runner` wraps an
  ``agentshim``-backed ``vibe_sys._agent_cli`` compatibility layer, which drives
...
build a fresh agent for each call (clean context window) and ``vibe_sys._agent_cli``
        ...
        *,
        ...
        Args:
            ...
            mcp_servers: Optional list of stdio MCP servers to install for
                ...
                :meth:`~vibe_sys._agent_cli.base.CodingAgent.install_mcp_servers`
