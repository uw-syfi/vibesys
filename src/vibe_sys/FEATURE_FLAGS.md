...
VibeSys declares feature flags in `src/vibe_sys/features.py`.
...
`src/vibe_sys/config.py` parses `[feature_flags]` from `agent.toml` with
...
```python
from vibe_sys.features import FeatureFlag, is_feature_enabled
...
```python
from vibe_sys.features import FEATURES, FeatureFlag
