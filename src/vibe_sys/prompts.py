...
- **Fragment** — a small reusable snippet meant to be composed *into*
  ...
  ``vibe_sys/templates/_backend/<backend>/<name>.j2``. The
...
See ``vibe_sys/templates/_backend/README.md`` for the
...
def _build_env(template_dir: Path | str | None = None) -> Environment:
    ...
    ``vibe_sys/templates/`` root, so ``{% include
...
class ComputeBackendFragment(ABC):
    ...
    ``vibe_sys/templates/_backend/<backend>/``.
...
class Prompt:
    ...
    template_dir:
        ...
        ``vibe_sys/templates/`` root, where backend fragments
        live.
    backend:
