...
`Prompt(template_dir, backend)` (in `vibe_sys/prompts.py`) auto-injects every fragment under `_backend/<backend>/` as a kwarg keyed by **filename stem** on every `prompt.render(...)` call.
...
1. Add the variant to `ComputeBackend` in
   `vibe_sys/constants.py`.
2. Create `vibe_sys/templates/_backend/<new>/` and mirror every
...
4. Wire up the backend's runtime impl under
   `vibe_sys/backends/<new>/` and register it in
   `backends/__init__.py`. See the existing CUDA and Metal impls for
   the pattern.

## Python contract

The canonical list of fragment names lives on `ComputeBackendFragment.NAMES` (in `vibe_sys/prompts.py`), not in this directory layout. Adding a fragment is a 3-step change:
