...
from vibe_database.schemas import ProfilerSummary


def mcp_spec(profiler_kind: str):
    """Build an ``MCPServerSpec`` that spawns the analysis MCP server.

    Returns ``None`` when ``vibe_database._agent_cli`` is not importable in the
    ...
    """
    try:
        from vibe_database._agent_cli import MCPServerSpec
    except Exception:
        return None
