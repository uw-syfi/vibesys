...
from vibe_database.config import Config, _load_config
...
def _parse_cli_objective(spec: str):
    ...
    from vibe_database.loops.evolve.population import Objective
...
def _run_evolve(args: argparse.Namespace) -> None:
    config, skills, backend = load_config_and_skills(args)
    from vibe_database.loops.evolve.loop import run_evolve_loop
...
def _run_openevolve(args: argparse.Namespace) -> None:
    config, skills, backend = load_config_and_skills(args)
    from vibe_database.loops.openevolve.loop import run_openevolve_loop
...
def main() -> None:
    ...
    # ``vibe_database.cli._{validate,run}_<kind>`` takes effect.
